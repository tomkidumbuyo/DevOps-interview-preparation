...
Operate the authoritative API, admission/storage path and consistent etcd quorum under bounded load.

```mermaid
flowchart LR
  I[identity and desired state] --> C[Kubernetes API server and etcd control plane]
  C --> D[Kubernetes API server and etcd data plane]
...
For Kubernetes API server and etcd, draw a real request/resource path and label where these mechanisms act: API groups/versions, Request path, Watch, ResourceVersion, Server-side apply, etcd quorum, Compaction, Defragmentation, Encryption at rest, API priority/fairness. State which parts are control plane versus data plane, regional versus zonal/global, synchronous versus asynchronous, and customer versus provider responsibility.
...
1. **Easy:** inventory one healthy Kubernetes API server and etcd resource and draw identity/control/data/dependency paths.
...
Explain Kubernetes API server and etcd in five passes: purpose/selection, mechanism/lifecycle, security/failure, operation/commands, and architecture/economics. Then complete the separate [answered question bank](questions-and-answers.md) without looking at these notes.
