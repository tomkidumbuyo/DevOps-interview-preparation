...
**Scheduler, controllers and kubelet** is easiest to understand as one part of a larger path. The subject is governed through an API-driven feedback system. A client writes desired state, controllers compare it with observed state, the scheduler chooses placement, and node agents and plugins produce the running data plane.

The chapter focuses on Controller loop, Scheduler queue, Filter plugin, Score plugin. These are connected mechanisms, not vocabulary to memorize. Trace desired state through reconciliation, binding and node execution without confusing component responsibilities The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **Scheduler, controllers and kubelet** is the next layer of that evolution. Its modern purpose is to trace desired state through reconciliation, binding and node execution without confusing component responsibilities. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["Scheduler, controllers and kubelet: node plugins and runtime"]
...
**What it is.** Rejects nodes that cannot satisfy resources, affinity, taints, topology or volumes.
...
**What it is.** Ranks feasible nodes and can favor spread/bin packing/locality.
...
**What it is.** Records selected node in Pod spec after scheduling assumptions/reservations.
...
**What it is.** Realizes assigned Pod through volume, sandbox, image and container runtime operations.
...
**What it is.** GRPC boundary from kubelet to containerd/CRI-O runtime.
...
**What it is.** Kubelet heartbeat and conditions drive availability/eviction decisions.
...
**What it is.** Reconciles nodes/routes/load balancers against cloud APIs.
...
**What it is.** Controllers/schedulers use leases so only active leader mutates shared state.
...
- `kubectl get lease -n kube-system` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
...
- `journalctl -u kubelet -u containerd --since '-30 min'` captures a read-oriented state snapshot that must be interpreted against a healthy baseline, the exact target and the next adjacent dependency.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **Scheduler, controllers and kubelet**, not merely a list to copy.
...
Build a small, safe proof of **Scheduler, controllers and kubelet** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
