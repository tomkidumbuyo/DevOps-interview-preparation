...
<!-- chapter-guide:start -->
> **Step 072 of 373 — 06.04**
>
> **Builds on:** [Service mesh](../03-networking/05-service-mesh/README.md)
>
> **Now:** Learn **Storage** from its mental model through production ownership.
>
> **Then:** Rehearse the linked questions and continue to [PersistentVolumes, PVCs and StorageClasses](01-pv-pvc-storageclass/README.md).
<!-- chapter-guide:end -->
...
flowchart LR
  B[Storage]
  B --> S1[PersistentVolumes, PVCs and StorageClasses]
  B --> S2[CSI, snapshots and Kubernetes backup]
```

## Branch learning contract

Learn the easy mental model first, run the read-only commands in a sandbox, render/apply the examples only in disposable environments, then break and repair one dependency at a time. Be able to connect these topics across the branch: PersistentVolume, PersistentVolumeClaim, StorageClass, CSI controller, CSI node plugin, CSIDriver/CSINode.
...
> Interview bank: [questions-and-answers.md](questions-and-answers.md) · Official documentation: <https://kubernetes.io/docs/concepts/storage/persistent-volumes/>

## Explanation

### What it is and why it exists

**Storage** is easiest to understand as one part of a larger path. The subject is governed through an API-driven feedback system. A client writes desired state, controllers compare it with observed state, the scheduler chooses placement, and node agents and plugins produce the running data plane.

The chapter focuses on PersistentVolume, PersistentVolumeClaim, CSI controller, CSI node plugin. These are connected mechanisms, not vocabulary to memorize. Integrate the storage branch as one production capability rather than isolated products The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **Storage** is the next layer of that evolution. Its modern purpose is to integrate the storage branch as one production capability rather than isolated products. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["Storage: node plugins and runtime"]
...
**What it is.** Namespaced request for class, capacity, access mode and volume mode.
...
**What it is.** External components provision/delete/attach/snapshot/resize volumes.
...
**What it is.** Stages/publishes/mounts volumes on each node.
...
- `kubectl get sc,pv,pvc -A` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl get csidriver,csinode,volumeattachment` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **Storage**, not merely a list to copy.
...
|---:|---|---|
| 1 | **PersistentVolume** | cluster object representing provisioned storage and reclaim policy |
| 2 | **PersistentVolumeClaim** | namespaced request for class, capacity, access mode and volume mode |
| 3 | **CSI controller** | external components provision/delete/attach/snapshot/resize volumes |
| 4 | **CSI node plugin** | stages/publishes/mounts volumes on each node |
...
Build a small, safe proof of **Storage** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
...
**Reading path:** [← Back: Service mesh](../03-networking/05-service-mesh/README.md) · [Questions](questions-and-answers.md) · [Next: PersistentVolumes, PVCs and StorageClasses →](01-pv-pvc-storageclass/README.md)
