...
<!-- chapter-guide:start -->
> **Step 061 of 373 — 06.02.01**
>
> **Builds on:** [Workloads](../README.md)
>
> **Now:** Learn **Kubernetes Pods** from its mental model through production ownership.
>
> **Then:** Rehearse the linked questions and continue to [Deployments and ReplicaSets](../02-deployments/README.md).
<!-- chapter-guide:end -->
...
**Kubernetes Pods** is easiest to understand as one part of a larger path. The subject is governed through an API-driven feedback system. A client writes desired state, controllers compare it with observed state, the scheduler chooses placement, and node agents and plugins produce the running data plane.

The chapter focuses on Pod sandbox, Phase, Init container, Sidecar. These are connected mechanisms, not vocabulary to memorize. Operate the shared scheduling/network/storage/security unit and its complete lifecycle The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **Kubernetes Pods** is the next layer of that evolution. Its modern purpose is to operate the shared scheduling/network/storage/security unit and its complete lifecycle. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["Kubernetes Pods: node plugins and runtime"]
...
For **Kubernetes Pods**, the mechanisms participating in that loop are Pod sandbox, Phase, Init container, Sidecar, Ephemeral container, Restart policy, Termination grace, Pod readiness gate, Owner reference, Ephemeral storage. Some run synchronously on the caller's request, while others converge later. This timing distinction explains many production surprises: the desired object can exist before capacity is ready, a data path can continue while its control plane is impaired, and a timeout can leave the final side effect unknown.

### Core concepts explained in detail

#### Pod sandbox

**What it is.** Containers share network namespace and can share IPC/process/volumes by configuration.
...
**What it is.** Pending/Running/Succeeded/Failed/Unknown is coarse and conditions/container states give real detail.
...
**What it is.** Sequential prerequisite that must complete before app containers.
...
**What it is.** Supporting container lifecycle/order needs explicit startup/termination semantics.
...
**What it is.** Debug process added to a running Pod without normal resources/probes/restart.
...
**What it is.** Kubelet restarts containers under Always/OnFailure/Never and exponential backoff.
...
**What it is.** SIGTERM/preStop/drain must finish before SIGKILL deadline.
...
**What it is.** Custom condition can delay readiness beyond container probes.
...
**What it is.** Connects Pod to controller garbage collection and desired count.
...
**What it is.** Writable layer/logs/emptyDir count toward requests/limits and node disk pressure.
...
- `kubectl get pod POD -n NS -o yaml` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl describe pod POD -n NS` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl logs POD -n NS --all-containers --previous` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl debug -it POD -n NS --image=nicolaka/netshoot --target=CONTAINER` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **Kubernetes Pods**, not merely a list to copy.
...
| 3 | **Init container** | sequential prerequisite that must complete before app containers |
| 4 | **Sidecar** | supporting container lifecycle/order needs explicit startup/termination semantics |
| 5 | **Ephemeral container** | debug process added to a running Pod without normal resources/probes/restart |
| 6 | **Restart policy** | kubelet restarts containers under Always/OnFailure/Never and exponential backoff |
| 7 | **Termination grace** | SIGTERM/preStop/drain must finish before SIGKILL deadline |
| 8 | **Pod readiness gate** | custom condition can delay readiness beyond container probes |
| 9 | **Owner reference** | connects Pod to controller garbage collection and desired count |
| 10 | **Ephemeral storage** | writable layer/logs/emptyDir count toward requests/limits and node disk pressure |
...
Build a small, safe proof of **Kubernetes Pods** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
...
**Reading path:** [← Back: Workloads](../README.md) · [Questions](questions-and-answers.md) · [Next: Deployments and ReplicaSets →](../02-deployments/README.md)
