...
<!-- chapter-guide:start -->
> **Step 062 of 373 — 06.02.02**
>
> **Builds on:** [Kubernetes Pods](../01-pods/README.md)
>
> **Now:** Learn **Deployments and ReplicaSets** from its mental model through production ownership.
>
> **Then:** Rehearse the linked questions and continue to [StatefulSets and DaemonSets](../03-statefulsets-daemonsets/README.md).
<!-- chapter-guide:end -->

> Interview bank: [questions-and-answers.md](questions-and-answers.md) · Official documentation: <https://kubernetes.io/docs/concepts/workloads/controllers/deployment/>

## Explanation

### What it is and why it exists

**Deployments and ReplicaSets** is easiest to understand as one part of a larger path. The subject is governed through an API-driven feedback system. A client writes desired state, controllers compare it with observed state, the scheduler chooses placement, and node agents and plugins produce the running data plane.

The chapter focuses on Deployment selector, Pod template hash, RollingUpdate, Recreate. These are connected mechanisms, not vocabulary to memorize. Perform safe stateless rollouts, pause/undo and diagnose progress/readiness without manual Pod ownership The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **Deployments and ReplicaSets** is the next layer of that evolution. Its modern purpose is to perform safe stateless rollouts, pause/undo and diagnose progress/readiness without manual Pod ownership. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["Deployments and ReplicaSets: node plugins and runtime"]
...
**What it is.** Creates a new ReplicaSet when template content changes.
...
**What it is.** MaxSurge/maxUnavailable bound temporary capacity and disruption.
...
**What it is.** Terminates old Pods before new when coexistence is unsafe.
...
**What it is.** Requires sustained readiness before a Pod counts available.
...
**What it is.** Marks rollout stalled but does not automatically roll back.
...
**What it is.** Retained ReplicaSets enable undo at storage/resource cost.
...
**What it is.** Batches template changes before a new rollout.
...
**What it is.** Desired/updated/ready/available replicas explain rollout state.
...
**What it is.** Readiness, preStop, grace and LB endpoint propagation protect in-flight work.
...
- `kubectl rollout status deployment/NAME -n NS` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl rollout history deployment/NAME -n NS` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl rollout undo deployment/NAME -n NS --to-revision=REV` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl get rs -n NS --sort-by=.metadata.creationTimestamp` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **Deployments and ReplicaSets**, not merely a list to copy.
...
|---:|---|---|
| 1 | **Deployment selector** | immutable label contract determines which ReplicaSets/Pods are owned |
| 2 | **Pod template hash** | creates a new ReplicaSet when template content changes |
| 3 | **RollingUpdate** | maxSurge/maxUnavailable bound temporary capacity and disruption |
| 4 | **Recreate** | terminates old Pods before new when coexistence is unsafe |
| 5 | **minReadySeconds** | requires sustained readiness before a Pod counts available |
| 6 | **progressDeadlineSeconds** | marks rollout stalled but does not automatically roll back |
| 7 | **Revision history** | retained ReplicaSets enable undo at storage/resource cost |
| 8 | **Pause/resume** | batches template changes before a new rollout |
| 9 | **Availability condition** | desired/updated/ready/available replicas explain rollout state |
| 10 | **Drain compatibility** | readiness, preStop, grace and LB endpoint propagation protect in-flight work |
...
Build a small, safe proof of **Deployments and ReplicaSets** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
...
**Reading path:** [← Back: Kubernetes Pods](../01-pods/README.md) · [Questions](questions-and-answers.md) · [Next: StatefulSets and DaemonSets →](../03-statefulsets-daemonsets/README.md)
