...
**StatefulSets and DaemonSets** is easiest to understand as one part of a larger path. The subject is governed through an API-driven feedback system. A client writes desired state, controllers compare it with observed state, the scheduler chooses placement, and node agents and plugins produce the running data plane.

The chapter focuses on Stateful ordinal, Headless Service, VolumeClaimTemplate, OrderedReady. These are connected mechanisms, not vocabulary to memorize. Operate stable-identity/stateful workloads and node-local agents under controlled update and disruption The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **StatefulSets and DaemonSets** is the next layer of that evolution. Its modern purpose is to operate stable-identity/stateful workloads and node-local agents under controlled update and disruption. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["StatefulSets and DaemonSets: node plugins and runtime"]
...
For **StatefulSets and DaemonSets**, the mechanisms participating in that loop are Stateful ordinal, Headless Service, VolumeClaimTemplate, OrderedReady, Partitioned update, State replication, DaemonSet eligibility, DaemonSet rollout, Critical agents, Node drain. Some run synchronously on the caller's request, while others converge later. This timing distinction explains many production surprises: the desired object can exist before capacity is ready, a data path can continue while its control plane is impaired, and a timeout can leave the final side effect unknown.

### Core concepts explained in detail

#### Stateful ordinal

**What it is.** Stable Pod name/identity persists across rescheduling but not physical node.
...
**What it is.** Publishes per-Pod DNS identity for StatefulSet membership.
...
**What it is.** Creates stable per-ordinal PVCs that are not automatically deleted by default semantics.
...
**What it is.** Sequential create/update assumes application readiness and can deadlock bad clusters.
...
**What it is.** Rolls only selected ordinals for canary/manual control.
...
**What it is.** Application/database must implement quorum/failover; StatefulSet does not.
...
**What it is.** Runs on every matching node and tolerates common node conditions through controller defaults.
...
**What it is.** MaxUnavailable/surge support varies by strategy/version and affects node agent coverage.
...
**What it is.** CNI/storage/log/GPU DaemonSets need priority/resources and upgrade ordering.
...
**What it is.** DaemonSet Pods are ignored/recreated, while stateful eviction must respect PDB/storage.
...
- `kubectl get statefulset,daemonset -A` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl rollout status statefulset/NAME -n NS` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl rollout status daemonset/NAME -n NS` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl get pvc -n NS -l app=NAME` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **StatefulSets and DaemonSets**, not merely a list to copy.
...
Build a small, safe proof of **StatefulSets and DaemonSets** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
