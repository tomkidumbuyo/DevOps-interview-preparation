...
**PDB, priority, preemption and eviction** is easiest to understand as one part of a larger path. The subject is governed through an API-driven feedback system. A client writes desired state, controllers compare it with observed state, the scheduler chooses placement, and node agents and plugins produce the running data plane.

The chapter focuses on PDB, Eviction API, Unhealthy eviction policy, PriorityClass. These are connected mechanisms, not vocabulary to memorize. Control voluntary disruption and overload scheduling without assuming protection from node failure The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **PDB, priority, preemption and eviction** is the next layer of that evolution. Its modern purpose is to control voluntary disruption and overload scheduling without assuming protection from node failure. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["PDB, priority, preemption and eviction: node plugins and runtime"]
...
For **PDB, priority, preemption and eviction**, the mechanisms participating in that loop are PDB, Eviction API, Unhealthy eviction policy, PriorityClass, Preemption, Nominated node, Node-pressure eviction, Taint eviction, Drain, Overprotection. Some run synchronously on the caller's request, while others converge later. This timing distinction explains many production surprises: the desired object can exist before capacity is ready, a data path can continue while its control plane is impaired, and a timeout can leave the final side effect unknown.
...
**What it is.** Controls drain behavior for unhealthy Pods under supported API.
...
**What it is.** Numeric scheduling/eviction priority with globalDefault/preemption policy.
...
**What it is.** Scheduler removes lower-priority Pods when that can make a high-priority Pod feasible.
...
**What it is.** Indicates expected preemption placement but can change before binding.
...
**What it is.** Kubelet reclaims resources based on signals/QoS/priority and is involuntary.
...
**What it is.** NoExecute taint can evict non-tolerating Pods after tolerationSeconds.
...
**What it is.** Cordons then evicts/deletes workload Pods while respecting flags/PDB.
...
**What it is.** Strict PDB/priority can block maintenance or starve lower-value platform services.
...
- `kubectl describe pdb NAME -n NS` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl drain NODE --ignore-daemonsets --dry-run=server` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl get events -A | rg 'Preempt|Evict|Disruption'` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **PDB, priority, preemption and eviction**, not merely a list to copy.
...
Build a small, safe proof of **PDB, priority, preemption and eviction** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
