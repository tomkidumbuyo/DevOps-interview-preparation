...
**HPA, VPA and KEDA** is easiest to understand as one part of a larger path. The subject is governed through an API-driven feedback system. A client writes desired state, controllers compare it with observed state, the scheduler chooses placement, and node agents and plugins produce the running data plane.

The chapter focuses on HPA ratio, Resource utilization, Custom/external metric, HPA behavior. These are connected mechanisms, not vocabulary to memorize. Scale replicas/resources from stable demand signals while avoiding feedback, metric loss and unsafe churn The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **HPA, VPA and KEDA** is the next layer of that evolution. Its modern purpose is to scale replicas/resources from stable demand signals while avoiding feedback, metric loss and unsafe churn. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["HPA, VPA and KEDA: node plugins and runtime"]
...
For **HPA, VPA and KEDA**, the mechanisms participating in that loop are HPA ratio, Resource utilization, Custom/external metric, HPA behavior, VPA recommendation, VPA/HPA interaction, KEDA trigger, Queue age/depth, Scale-to-zero, Downstream protection. Some run synchronously on the caller's request, while others converge later. This timing distinction explains many production surprises: the desired object can exist before capacity is ready, a data path can continue while its control plane is impaired, and a timeout can leave the final side effect unknown.
...
**What it is.** Adapter exposes application/provider signal with availability/cardinality semantics.
...
**What it is.** Stabilization and policies bound scale-up/down rate.
...
**What it is.** Learns requests from usage history and can restart Pods to apply changes.
...
**What it is.** Both changing CPU requests/replicas can create feedback without design.
...
**What it is.** Event source drives Scale subresource and supports activation/scale-to-zero.
...
**What it is.** Leading demand signal but must account for service time/token work.
...
**What it is.** Cold start and missing metric/activation path must meet SLO.
...
**What it is.** Replica scale cannot exceed database/provider/tenant capacity.
...
- `kubectl describe hpa NAME -n NS` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl get --raw /apis/custom.metrics.k8s.io/v1beta1 | jq` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl get scaledobject -A` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl get vpa -A` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **HPA, VPA and KEDA**, not merely a list to copy.
...
Build a small, safe proof of **HPA, VPA and KEDA** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
