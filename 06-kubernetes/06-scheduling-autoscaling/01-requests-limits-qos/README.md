...
<!-- chapter-guide:start -->
> **Step 082 of 373 — 06.06.01**
>
> **Builds on:** [Scheduling Autoscaling](../README.md)
>
> **Now:** Learn **Resource requests, limits and QoS** from its mental model through production ownership.
>
> **Then:** Rehearse the linked questions and continue to [Affinity, taints and topology placement](../02-placement/README.md).
<!-- chapter-guide:end -->
...
**Resource requests, limits and QoS** is easiest to understand as one part of a larger path. The subject is governed through an API-driven feedback system. A client writes desired state, controllers compare it with observed state, the scheduler chooses placement, and node agents and plugins produce the running data plane.

The chapter focuses on CPU request, CPU limit, Memory request, Memory limit. These are connected mechanisms, not vocabulary to memorize. Schedule and isolate CPU, memory, ephemeral storage and extended resources from measured workload needs The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **Resource requests, limits and QoS** is the next layer of that evolution. Its modern purpose is to schedule and isolate CPU, memory, ephemeral storage and extended resources from measured workload needs. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["Resource requests, limits and QoS: node plugins and runtime"]
...
For **Resource requests, limits and QoS**, the mechanisms participating in that loop are CPU request, CPU limit, Memory request, Memory limit, Ephemeral-storage, Guaranteed QoS, Burstable QoS, BestEffort QoS, LimitRange, ResourceQuota. Some run synchronously on the caller's request, while others converge later. This timing distinction explains many production surprises: the desired object can exist before capacity is ready, a data path can continue while its control plane is impaired, and a timeout can leave the final side effect unknown.

### Core concepts explained in detail

#### CPU request

**What it is.** Scheduler reservation and HPA utilization denominator, expressed in cores/millicores.
...
**What it is.** Cgroup bandwidth cap can throttle latency-sensitive work even with idle host cores.
...
**What it is.** Scheduling reservation/eviction signal rather than guaranteed preallocation.
...
**What it is.** Cgroup hard boundary can OOM-kill a container.
...
**What it is.** Writable layer/log/emptyDir request/limit interacts with node disk eviction.
...
**What it is.** Equal CPU/memory request/limit on every container gives strongest eviction priority.
...
**What it is.** At least one request/limit but not Guaranteed; common production class.
...
**What it is.** No requests/limits and first under resource pressure.
...
**What it is.** Injects/enforces per-container defaults and bounds.
...
**What it is.** Namespace aggregate limits can reject objects or cap priority/storage/GPU counts.
...
- `kubectl describe node NODE` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl get pod POD -o jsonpath='{.status.qosClass}'` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl get resourcequota,limitrange -A` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **Resource requests, limits and QoS**, not merely a list to copy.
...
Build a small, safe proof of **Resource requests, limits and QoS** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
...
**Reading path:** [← Back: Scheduling Autoscaling](../README.md) · [Questions](questions-and-answers.md) · [Next: Affinity, taints and topology placement →](../02-placement/README.md)
