...
**Scheduling Autoscaling** is easiest to understand as one part of a larger path. The subject is governed through an API-driven feedback system. A client writes desired state, controllers compare it with observed state, the scheduler chooses placement, and node agents and plugins produce the running data plane.

The chapter focuses on CPU request, CPU limit, nodeSelector, Node affinity. These are connected mechanisms, not vocabulary to memorize. Integrate the scheduling autoscaling branch as one production capability rather than isolated products The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **Scheduling Autoscaling** is the next layer of that evolution. Its modern purpose is to integrate the scheduling autoscaling branch as one production capability rather than isolated products. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["Scheduling Autoscaling: node plugins and runtime"]
...
**What it is.** Exact label match for simple required placement.
...
**What it is.** Expressive required/preferred label predicates evaluated at scheduling.
...
**What it is.** Desired replicas derives current/target metric with tolerance and missing/not-ready handling.
...
**What it is.** Usage divided by requests means bad/missing requests break CPU/memory scaling.
...
**What it is.** Requests/constraints are the node capacity specification.
...
**What it is.** Cluster Autoscaler simulates predefined group capacity/labels/taints.
...
**What it is.** MinAvailable/maxUnavailable bounds voluntary API eviction for selected healthy Pods.
...
**What it is.** Admission checks PDB and graceful deletion compared with direct delete.
...
- `kubectl get nodes --show-labels` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl get hpa -A` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
...
- `kubectl get pdb,priorityclass -A` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **Scheduling Autoscaling**, not merely a list to copy.
...
Build a small, safe proof of **Scheduling Autoscaling** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
