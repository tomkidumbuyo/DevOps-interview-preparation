...
<!-- chapter-guide:start -->
> **Step 068 of 373 — 06.03.02**
>
> **Builds on:** [Services, EndpointSlices and CoreDNS](../01-services-endpoints-dns/README.md)
>
> **Now:** Learn **CNI, kube-proxy and eBPF data planes** from its mental model through production ownership.
>
> **Then:** Rehearse the linked questions and continue to [Kubernetes NetworkPolicy](../03-networkpolicy/README.md).
<!-- chapter-guide:end -->
...
**CNI, kube-proxy and eBPF data planes** is easiest to understand as one part of a larger path. The subject is governed through an API-driven feedback system. A client writes desired state, controllers compare it with observed state, the scheduler chooses placement, and node agents and plugins produce the running data plane.

The chapter focuses on Kubernetes network model, CNI plugin, IPAM, kube-proxy iptables. These are connected mechanisms, not vocabulary to memorize. Implement Pod addressing/routing and Service translation under IP, conntrack and policy constraints The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **CNI, kube-proxy and eBPF data planes** is the next layer of that evolution. Its modern purpose is to implement Pod addressing/routing and Service translation under IP, conntrack and policy constraints. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["CNI, kube-proxy and eBPF data planes: node plugins and runtime"]
...
For **CNI, kube-proxy and eBPF data planes**, the mechanisms participating in that loop are Kubernetes network model, CNI plugin, IPAM, kube-proxy iptables, kube-proxy IPVS, eBPF data plane, Conntrack, MTU, SNAT/source preservation, Node-to-Pod routing. Some run synchronously on the caller's request, while others converge later. This timing distinction explains many production surprises: the desired object can exist before capacity is ready, a data path can continue while its control plane is impaired, and a timeout can leave the final side effect unknown.

### Core concepts explained in detail

#### Kubernetes network model

**What it is.** Pods have cluster-routable IP identity and communicate without required NAT inside the model.
...
**What it is.** Configures Pod interface, address, route and cleanup when runtime creates sandbox.
...
**What it is.** Allocates/reclaims Pod addresses and can exhaust/fragment subnet pools.
...
**What it is.** Programs probabilistic NAT rules for Services and endpoints.
...
**What it is.** Uses kernel virtual server tables with separate synchronization.
...
**What it is.** Programs kernel hooks/maps for routing, load balancing, policy and observability.
...
**What it is.** Tracks NAT/state and can exhaust or retain stale translations.
...
**What it is.** Encapsulation/cloud path mismatch can drop large packets while probes pass.
...
#### SNAT/source preservation

**What it is.** External/local traffic policy and plugin behavior influence client IP.
...
**What it is.** Overlay, native routing or cloud ENIs change troubleshooting and scale limits.
...
- `kubectl get ds -n kube-system` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl logs -n kube-system DS_POD --since=30m` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `ip route && ip link` captures a read-oriented state snapshot that must be interpreted against a healthy baseline, the exact target and the next adjacent dependency.
- `conntrack -S` captures a read-oriented state snapshot that must be interpreted against a healthy baseline, the exact target and the next adjacent dependency.
- `bpftool map list` captures a read-oriented state snapshot that must be interpreted against a healthy baseline, the exact target and the next adjacent dependency.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **CNI, kube-proxy and eBPF data planes**, not merely a list to copy.
...
Build a small, safe proof of **CNI, kube-proxy and eBPF data planes** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
...
**Reading path:** [← Back: Services, EndpointSlices and CoreDNS](../01-services-endpoints-dns/README.md) · [Questions](questions-and-answers.md) · [Next: Kubernetes NetworkPolicy →](../03-networkpolicy/README.md)
