...
<!-- chapter-guide:start -->
> **Step 070 of 373 — 06.03.04**
>
> **Builds on:** [Kubernetes NetworkPolicy](../03-networkpolicy/README.md)
>
> **Now:** Learn **Ingress and Gateway API** from its mental model through production ownership.
>
> **Then:** Rehearse the linked questions and continue to [Service mesh](../05-service-mesh/README.md).
<!-- chapter-guide:end -->
...
**Ingress and Gateway API** is easiest to understand as one part of a larger path. The subject is governed through an API-driven feedback system. A client writes desired state, controllers compare it with observed state, the scheduler chooses placement, and node agents and plugins produce the running data plane.

The chapter focuses on IngressClass/controller, Ingress rule, GatewayClass, Gateway/listener. These are connected mechanisms, not vocabulary to memorize. Separate infrastructure listeners from application routing and operate TLS, status, policy and progressive traffic safely The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **Ingress and Gateway API** is the next layer of that evolution. Its modern purpose is to separate infrastructure listeners from application routing and operate TLS, status, policy and progressive traffic safely. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["Ingress and Gateway API: node plugins and runtime"]
...
For **Ingress and Gateway API**, the mechanisms participating in that loop are IngressClass/controller, Ingress rule, GatewayClass, Gateway/listener, HTTPRoute/GRPCRoute, Parent status, ReferenceGrant, TLS termination/passthrough, Timeout/retry policy, Traffic split. Some run synchronously on the caller's request, while others converge later. This timing distinction explains many production surprises: the desired object can exist before capacity is ready, a data path can continue while its control plane is impaired, and a timeout can leave the final side effect unknown.

### Core concepts explained in detail

#### IngressClass/controller

**What it is.** Selects implementation and controller-specific features.
...
**What it is.** Host/path routes HTTP(S) to a Service backend under limited portable API.
...
**What it is.** Cluster-scoped implementation class controlled by infrastructure owner.
...
**What it is.** Address/protocol/port/hostname/TLS attachment owned by platform/network team.
...
**What it is.** Application route match/filter/backend and weighted traffic.
...
**What it is.** Accepted/ResolvedRefs/Programmed conditions reveal attachment/reconcile state.
...
**What it is.** Explicitly authorizes supported cross-namespace references.
...
**What it is.** Route visibility, certificate and backend encryption differ.
...
**What it is.** Implementation/API support must align with streaming and end-to-end deadlines.
...
**What it is.** Weights need metrics/eval/stickiness and rollback rather than assumption.
...
curl -vk --resolve HOST:443:ADDRESS https://HOST/PATH
```

What the example demonstrates:

- `kubectl get ingressclass,ingress,gatewayclass,gateway,httproute,grpcroute -A` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl describe gateway NAME -n NS` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl get httproute NAME -n NS -o jsonpath='{.status.parents}' | jq` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `curl -vk --resolve HOST:443:ADDRESS https://HOST/PATH` tests one part of the end-to-end request path and preserves protocol-level evidence such as resolution, connection, handshake, status or packet direction.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **Ingress and Gateway API**, not merely a list to copy.
...
|---:|---|---|
| 1 | **IngressClass/controller** | selects implementation and controller-specific features |
| 2 | **Ingress rule** | host/path routes HTTP(S) to a Service backend under limited portable API |
| 3 | **GatewayClass** | cluster-scoped implementation class controlled by infrastructure owner |
| 4 | **Gateway/listener** | address/protocol/port/hostname/TLS attachment owned by platform/network team |
| 5 | **HTTPRoute/GRPCRoute** | application route match/filter/backend and weighted traffic |
| 6 | **Parent status** | Accepted/ResolvedRefs/Programmed conditions reveal attachment/reconcile state |
| 7 | **ReferenceGrant** | explicitly authorizes supported cross-namespace references |
| 8 | **TLS termination/passthrough** | route visibility, certificate and backend encryption differ |
| 9 | **Timeout/retry policy** | implementation/API support must align with streaming and end-to-end deadlines |
| 10 | **Traffic split** | weights need metrics/eval/stickiness and rollback rather than assumption |
...
Build a small, safe proof of **Ingress and Gateway API** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
...
**Reading path:** [← Back: Kubernetes NetworkPolicy](../03-networkpolicy/README.md) · [Questions](questions-and-answers.md) · [Next: Service mesh →](../05-service-mesh/README.md)
