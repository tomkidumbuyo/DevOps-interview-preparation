...
**Kubernetes NetworkPolicy** is easiest to understand as one part of a larger path. The subject is governed through an API-driven feedback system. A client writes desired state, controllers compare it with observed state, the scheduler chooses placement, and node agents and plugins produce the running data plane.

The chapter focuses on Pod selection, Isolation direction, Additive policy, Namespace selector. These are connected mechanisms, not vocabulary to memorize. Implement namespace/workload ingress and egress allow contracts with an enforcing CNI and verifiable DNS/dependency paths The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **Kubernetes NetworkPolicy** is the next layer of that evolution. Its modern purpose is to implement namespace/workload ingress and egress allow contracts with an enforcing CNI and verifiable DNS/dependency paths. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["Kubernetes NetworkPolicy: node plugins and runtime"]
...
**What it is.** All matching policy allows are unioned; deny rules are not in base API.
...
**What it is.** Selects labeled namespaces and needs protected namespace label governance.
...
**What it is.** Selects workload labels in the policy/selected namespace context.
...
**What it is.** CIDR/except matches IPs and can behave unexpectedly after NAT.
...
**What it is.** L3/L4 policy cannot natively authorize DNS names or HTTP paths.
...
**What it is.** Empty allow list establishes isolation and requires DNS/control/dependency allowances.
...
**What it is.** Source egress and destination ingress can independently block one flow.
...
**What it is.** Unsupported/no plugin means accepted objects may have no runtime effect.
...
- `kubectl describe networkpolicy NAME -n NS` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl run netshoot --rm -it -n NS --image=nicolaka/netshoot -- bash` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
...
A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **Kubernetes NetworkPolicy**, not merely a list to copy.
...
Build a small, safe proof of **Kubernetes NetworkPolicy** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
