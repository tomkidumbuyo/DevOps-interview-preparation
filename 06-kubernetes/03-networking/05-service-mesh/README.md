...
**Service mesh** is easiest to understand as one part of a larger path. The subject is governed through an API-driven feedback system. A client writes desired state, controllers compare it with observed state, the scheduler chooses placement, and node agents and plugins produce the running data plane.

The chapter focuses on Data-plane proxy, Control plane, Workload identity, mTLS. These are connected mechanisms, not vocabulary to memorize. Add workload identity, mTLS, traffic policy and telemetry without losing application/network ownership or overloading sidecars/control planes The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **Service mesh** is the next layer of that evolution. Its modern purpose is to add workload identity, mTLS, traffic policy and telemetry without losing application/network ownership or overloading sidecars/control planes. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["Service mesh: node plugins and runtime"]
...
**What it is.** Trust-domain/service-account identity authenticates peers rather than IP alone.
...
**What it is.** Encrypts/authenticates service traffic while certificate issuance/rotation becomes critical.
...
**What it is.** L4/L7 service identity rules complement Kubernetes NetworkPolicy.
...
**What it is.** Retries/timeouts/circuit/outlier/load balancing can amplify or mask application behavior.
...
**What it is.** Proxy metrics/traces improve path visibility at cardinality/resource cost.
...
**What it is.** Workloads missing sidecar/ambient enrollment can bypass expected controls.
...
**What it is.** Existing proxy config may continue while new workloads/certs/routes degrade.
...
**What it is.** Proxy/control/API skew and canary revision migration prevent fleet-wide breakage.
...
- `istioctl proxy-status` captures a read-oriented state snapshot that must be interpreted against a healthy baseline, the exact target and the next adjacent dependency.
- `istioctl analyze -A` captures a read-oriented state snapshot that must be interpreted against a healthy baseline, the exact target and the next adjacent dependency.
- `linkerd check` captures a read-oriented state snapshot that must be interpreted against a healthy baseline, the exact target and the next adjacent dependency.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **Service mesh**, not merely a list to copy.
...
Build a small, safe proof of **Service mesh** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
