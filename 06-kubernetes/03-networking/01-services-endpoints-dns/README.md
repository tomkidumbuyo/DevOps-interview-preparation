...
**Services, EndpointSlices and CoreDNS** is easiest to understand as one part of a larger path. The subject is governed through an API-driven feedback system. A client writes desired state, controllers compare it with observed state, the scheduler chooses placement, and node agents and plugins produce the running data plane.

The chapter focuses on ClusterIP, NodePort, LoadBalancer, ExternalName. These are connected mechanisms, not vocabulary to memorize. Route stable virtual service names to ready Pod endpoints and diagnose DNS/data-plane state separately The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **Services, EndpointSlices and CoreDNS** is the next layer of that evolution. Its modern purpose is to route stable virtual service names to ready Pod endpoints and diagnose DNS/data-plane state separately. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["Services, EndpointSlices and CoreDNS: node plugins and runtime"]
...
For **Services, EndpointSlices and CoreDNS**, the mechanisms participating in that loop are ClusterIP, NodePort, LoadBalancer, ExternalName, Headless Service, EndpointSlice, Service selector, targetPort, CoreDNS, DNS search/ndots. Some run synchronously on the caller's request, while others converge later. This timing distinction explains many production surprises: the desired object can exist before capacity is ready, a data path can continue while its control plane is impaired, and a timeout can leave the final side effect unknown.

### Core concepts explained in detail

#### ClusterIP

**What it is.** Virtual internal IP implemented by the cluster service data plane.
...
**What it is.** Reserves a port on nodes and routes to Service endpoints.
...
**What it is.** Asks a controller/cloud to expose a Service and report ingress status.
...
**What it is.** DNS CNAME indirection without proxying or health.
...
**What it is.** Returns endpoint DNS records without ClusterIP load balancing.
...
**What it is.** Scalable source of endpoint addresses, ports, readiness and topology hints.
...
**What it is.** Label query chooses Pods; mismatch yields no endpoints.
...
**What it is.** Number or named Pod port must match actual listener.
...
**What it is.** Watches Services/EndpointSlices and answers cluster zones plus forwarded external DNS.
...
**What it is.** Resolver expansion can create latency and surprising external/internal queries.
...
- `kubectl get svc,endpoints,endpointslice -A -o wide` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl describe svc NAME -n NS` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl run dns-test --rm -it --image=nicolaka/netshoot -- dig SVC.NS.svc.cluster.local` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl logs -n kube-system -l k8s-app=kube-dns --since=30m` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **Services, EndpointSlices and CoreDNS**, not merely a list to copy.
...
Build a small, safe proof of **Services, EndpointSlices and CoreDNS** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
