# Pod and node workload security

<!-- chapter-guide:start -->
> **Step 077 of 373 — 06.05.02**
>
> **Builds on:** [Kubernetes authentication, RBAC and ServiceAccounts](../01-authn-rbac-serviceaccounts/README.md)
>
> **Now:** Learn **Pod and node workload security** from its mental model through production ownership.
>
> **Then:** Rehearse the linked questions and continue to [Kubernetes Secrets and ConfigMaps](../03-secrets-configmaps/README.md).
<!-- chapter-guide:end -->

> Interview bank: [questions-and-answers.md](questions-and-answers.md) · Official documentation: <https://kubernetes.io/docs/concepts/security/pod-security-standards/>

## Explanation

### What it is and why it exists

**Pod and node workload security** is easiest to understand as one part of a larger path. The subject is governed through an API-driven feedback system. A client writes desired state, controllers compare it with observed state, the scheduler chooses placement, and node agents and plugins produce the running data plane.

The chapter focuses on runAsNonRoot, allowPrivilegeEscalation, Capabilities, Seccomp. These are connected mechanisms, not vocabulary to memorize. Constrain container privilege, host access, syscalls, identities and filesystems through safe defaults and admission The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **Pod and node workload security** is the next layer of that evolution. Its modern purpose is to constrain container privilege, host access, syscalls, identities and filesystems through safe defaults and admission. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["Pod and node workload security: node plugins and runtime"]
...
For **Pod and node workload security**, the mechanisms participating in that loop are runAsNonRoot, allowPrivilegeEscalation, Capabilities, Seccomp, Read-only root, Privileged container, Host namespaces/path, Pod Security Standards, Pod Security Admission, Node hardening. Some run synchronously on the caller's request, while others converge later. This timing distinction explains many production surprises: the desired object can exist before capacity is ready, a data path can continue while its control plane is impaired, and a timeout can leave the final side effect unknown.

### Core concepts explained in detail

#### runAsNonRoot

**What it is.** Prevents UID 0 execution when image/runtime identity can be determined.
...
**What it is.** Blocks gaining more privilege through exec/setuid and is affected by privilege/capabilities.
...
**What it is.** Drop ALL and add only exact kernel privileges required.
...
**What it is.** RuntimeDefault/local profiles reduce syscall attack surface.
...
**What it is.** Prevents image filesystem mutation and requires explicit writable volumes.
...
**What it is.** Largely bypasses container isolation and is equivalent to host-level trust.
...
#### Host namespaces/path

**What it is.** HostNetwork/PID/IPC/hostPath expose shared host attack surface.
...
**What it is.** Privileged/baseline/restricted profiles define portable control sets.
...
**What it is.** Namespace labels enforce/audit/warn standards by version.
...
**What it is.** Kernel/runtime/kubelet/metadata/patch and tenant separation remain outside Pod spec.
...
kubectl get pods -A -o json | jq '.items[] | select(any(.spec.containers[]; .securityContext.privileged==true))'
kubectl apply --dry-run=server -f pod.yaml
```

What the example demonstrates:

- `kubectl get ns --show-labels` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl label ns NS pod-security.kubernetes.io/enforce=restricted` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl get pods -A -o json | jq '.items[] | select(any(.spec.containers[]; .securityContext.privileged==true))'` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl apply --dry-run=server -f pod.yaml` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **Pod and node workload security**, not merely a list to copy.
...
|---:|---|---|
| 1 | **runAsNonRoot** | prevents UID 0 execution when image/runtime identity can be determined |
| 2 | **allowPrivilegeEscalation** | blocks gaining more privilege through exec/setuid and is affected by privilege/capabilities |
| 3 | **Capabilities** | drop ALL and add only exact kernel privileges required |
| 4 | **Seccomp** | RuntimeDefault/local profiles reduce syscall attack surface |
| 5 | **Read-only root** | prevents image filesystem mutation and requires explicit writable volumes |
| 6 | **Privileged container** | largely bypasses container isolation and is equivalent to host-level trust |
| 7 | **Host namespaces/path** | hostNetwork/PID/IPC/hostPath expose shared host attack surface |
| 8 | **Pod Security Standards** | privileged/baseline/restricted profiles define portable control sets |
| 9 | **Pod Security Admission** | namespace labels enforce/audit/warn standards by version |
| 10 | **Node hardening** | kernel/runtime/kubelet/metadata/patch and tenant separation remain outside Pod spec |
...
Build a small, safe proof of **Pod and node workload security** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
...
**Reading path:** [← Back: Kubernetes authentication, RBAC and ServiceAccounts](../01-authn-rbac-serviceaccounts/README.md) · [Questions](questions-and-answers.md) · [Next: Kubernetes Secrets and ConfigMaps →](../03-secrets-configmaps/README.md)
