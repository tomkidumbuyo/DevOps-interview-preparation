...
**Security** is easiest to understand as one part of a larger path. The subject is governed through an API-driven feedback system. A client writes desired state, controllers compare it with observed state, the scheduler chooses placement, and node agents and plugins produce the running data plane.

The chapter focuses on Authentication, Authorization, runAsNonRoot, allowPrivilegeEscalation. These are connected mechanisms, not vocabulary to memorize. Integrate the security branch as one production capability rather than isolated products The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **Security** is the next layer of that evolution. Its modern purpose is to integrate the security branch as one production capability rather than isolated products. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["Security: node plugins and runtime"]
...
**What it is.** Non-secret key/file configuration consumed via API, env or projected volume.
...
**What it is.** API object whose data is base64-encoded and requires encryption/RBAC controls.
...
**What it is.** Mutation precedes object schema/defaulting stages and validation before persistence.
...
**What it is.** CEL policy in API server avoids external webhook network dependency.
...
**What it is.** Soft/shared control boundary needing RBAC, policy, quotas and trusted cluster admins.
...
**What it is.** Stronger failure/control isolation at fleet cost and operational scale.
...
- `kubectl get configmap,secret -n NS` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl get validatingadmissionpolicy,validatingadmissionpolicybinding` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl get resourcequota,limitrange,networkpolicy,rolebinding -A` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **Security**, not merely a list to copy.
...
Build a small, safe proof of **Security** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
