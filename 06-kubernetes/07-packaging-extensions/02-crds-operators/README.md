...
**CRDs and operators** is easiest to understand as one part of a larger path. The subject is governed through an API-driven feedback system. A client writes desired state, controllers compare it with observed state, the scheduler chooses placement, and node agents and plugins produce the running data plane.

The chapter focuses on CRD schema, Reconciler, Status subresource, Finalizer. These are connected mechanisms, not vocabulary to memorize. Extend Kubernetes with versioned APIs and idempotent controllers that report status and clean external state safely The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **CRDs and operators** is the next layer of that evolution. Its modern purpose is to extend Kubernetes with versioned APIs and idempotent controllers that report status and clean external state safely. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["CRDs and operators: node plugins and runtime"]
...
**What it is.** Controller reports conditions/observedGeneration without mutating spec.
...
**What it is.** Ensures external cleanup before deletion completes and can stick when controller fails.
...
**What it is.** Drives garbage collection for Kubernetes child resources.
...
**What it is.** Event keys, rate limiting and retries coalesce reconciliation.
...
**What it is.** Default/validate/convert custom resources with HA/certificate/timeout needs.
...
**What it is.** Served/storage versions and conversion preserve compatibility.
...
**What it is.** Only one active replica performs shared mutation while others stand by.
...
**What it is.** Controller watches and permissions should be limited to owned resources/tenants.
...
- `kubectl explain CRD_KIND.spec --recursive` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl get CRD_KIND NAME -o yaml` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl get events --field-selector=involvedObject.kind=CRD_KIND` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl logs deploy/CONTROLLER --since=30m` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **CRDs and operators**, not merely a list to copy.
...
Build a small, safe proof of **CRDs and operators** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
