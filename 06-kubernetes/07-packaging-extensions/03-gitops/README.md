...
<!-- chapter-guide:start -->
> **Step 090 of 373 — 06.07.03**
>
> **Builds on:** [CRDs and operators](../02-crds-operators/README.md)
>
> **Now:** Learn **Argo CD, Flux and GitOps** from its mental model through production ownership.
>
> **Then:** Rehearse the linked questions and continue to [Operations](../../08-operations/README.md).
<!-- chapter-guide:end -->
...
**Argo CD, Flux and GitOps** is easiest to understand as one part of a larger path. The subject is governed through an API-driven feedback system. A client writes desired state, controllers compare it with observed state, the scheduler chooses placement, and node agents and plugins produce the running data plane.

The chapter focuses on Desired source, Reconciliation, Health, Sync wave/hook. These are connected mechanisms, not vocabulary to memorize. Continuously reconcile signed reviewed desired state across clusters with explicit tenancy, promotion, secrets and emergency workflow The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **Argo CD, Flux and GitOps** is the next layer of that evolution. Its modern purpose is to continuously reconcile signed reviewed desired state across clusters with explicit tenancy, promotion, secrets and emergency workflow. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["Argo CD, Flux and GitOps: node plugins and runtime"]
...
For **Argo CD, Flux and GitOps**, the mechanisms participating in that loop are Desired source, Reconciliation, Health, Sync wave/hook, ApplicationSet/Flux Kustomization, Project/tenant boundary, Prune/self-heal, Secret workflow, Promotion, Emergency change. Some run synchronously on the caller's request, while others converge later. This timing distinction explains many production surprises: the desired object can exist before capacity is ready, a data path can continue while its control plane is impaired, and a timeout can leave the final side effect unknown.

### Core concepts explained in detail

#### Desired source

**What it is.** Git/OCI revision plus render inputs is the audit/reconciliation truth.
...
**What it is.** Controller compares live and desired then applies/prunes under policy.
...
**What it is.** Resource-specific status estimates rollout but needs user/SLO verification.
...
**What it is.** Orders dependencies/migrations with failure and idempotency concerns.
...
#### ApplicationSet/Flux Kustomization

**What it is.** Generates fleet/environment instances from controlled templates.
...
**What it is.** Limits source repositories, clusters/namespaces and resource kinds.
...
**What it is.** Deletes drifted/removed resources and can amplify a bad commit.
...
**What it is.** Encrypted/external secret references prevent plaintext Git while preserving rotation.
...
**What it is.** Pull request updates immutable artifact/version between environment sources.
...
**What it is.** Pause/reconcile source quickly so controller does not revert mitigation.
...
- `argocd app diff APP` captures a read-oriented state snapshot that must be interpreted against a healthy baseline, the exact target and the next adjacent dependency.
- `argocd app sync APP --revision REV` captures a read-oriented state snapshot that must be interpreted against a healthy baseline, the exact target and the next adjacent dependency.
- `argocd app wait APP --health --sync` captures a read-oriented state snapshot that must be interpreted against a healthy baseline, the exact target and the next adjacent dependency.
- `flux get kustomizations -A` captures a read-oriented state snapshot that must be interpreted against a healthy baseline, the exact target and the next adjacent dependency.
- `flux reconcile kustomization NAME --with-source` captures a read-oriented state snapshot that must be interpreted against a healthy baseline, the exact target and the next adjacent dependency.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **Argo CD, Flux and GitOps**, not merely a list to copy.
...
Build a small, safe proof of **Argo CD, Flux and GitOps** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
...
**Reading path:** [← Back: CRDs and operators](../02-crds-operators/README.md) · [Questions](questions-and-answers.md) · [Next: Operations →](../../08-operations/README.md)
