...
**GPU sharing, scheduling and queueing** is easiest to understand as one part of a larger path. The subject is governed through an API-driven feedback system. A client writes desired state, controllers compare it with observed state, the scheduler chooses placement, and node agents and plugins produce the running data plane.

The chapter focuses on Whole GPU, MIG, Time-slicing, CUDA MPS. These are connected mechanisms, not vocabulary to memorize. Choose whole GPU, MIG, time-slicing or MPS and schedule scarce topology-aware capacity fairly and safely The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **GPU sharing, scheduling and queueing** is the next layer of that evolution. Its modern purpose is to choose whole GPU, MIG, time-slicing or MPS and schedule scarce topology-aware capacity fairly and safely. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["GPU sharing, scheduling and queueing: node plugins and runtime"]
...
For **GPU sharing, scheduling and queueing**, the mechanisms participating in that loop are Whole GPU, MIG, Time-slicing, CUDA MPS, GPU fragmentation, Gang scheduling, Fair sharing, Topology, Priority/preemption, Quota/accounting. Some run synchronously on the caller's request, while others converge later. This timing distinction explains many production surprises: the desired object can exist before capacity is ready, a data path can continue while its control plane is impaired, and a timeout can leave the final side effect unknown.

### Core concepts explained in detail

#### Whole GPU

**What it is.** Simple exclusive allocation offers strongest default isolation at fragmentation cost.
...
**What it is.** Supported GPUs partition hardware memory/fault domains into fixed profiles.
...
**What it is.** Oversubscribes compute without memory/fault isolation and weakens attribution.
...
**What it is.** Coordinates compatible processes for concurrency with separate operational/isolation constraints.
...
**What it is.** Free devices/profiles spread across nodes may not fit multi-GPU Pods.
...
**What it is.** Admits all workers/resources together so distributed jobs do not deadlock holding partial capacity.
...
**What it is.** Queues/cohorts borrow quota while preventing one tenant from monopolizing GPUs.
...
**What it is.** NUMA/PCIe/NVLink/NVSwitch/NIC placement determines collective/serving performance.
...
**What it is.** Scarce urgent workloads need policy, checkpoint/drain and starvation protection.
...
**What it is.** Tenant GPU/profile/time/token budgets need admission and usage measurement.
...
- `kubectl describe node GPU_NODE` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
...
- `nvidia-smi mig -lgi` shows the host-visible GPU, driver, topology or workload state; it proves hardware visibility but not framework compatibility or useful model goodput.
- `kubectl get workload,localqueue,clusterqueue -A` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **GPU sharing, scheduling and queueing**, not merely a list to copy.
...
Build a small, safe proof of **GPU sharing, scheduling and queueing** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
