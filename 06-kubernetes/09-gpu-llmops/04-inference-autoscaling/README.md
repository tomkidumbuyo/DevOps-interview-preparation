...
**Inference autoscaling and capacity** is easiest to understand as one part of a larger path. The subject is governed through an API-driven feedback system. A client writes desired state, controllers compare it with observed state, the scheduler chooses placement, and node agents and plugins produce the running data plane.

The chapter focuses on Queue depth/age, Predicted token work, TTFT/inter-token, KV cache pressure. These are connected mechanisms, not vocabulary to memorize. Scale model replicas and accelerator nodes from token queue and SLO while accounting for long cold starts and scarce capacity The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **Inference autoscaling and capacity** is the next layer of that evolution. Its modern purpose is to scale model replicas and accelerator nodes from token queue and SLO while accounting for long cold starts and scarce capacity. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["Inference autoscaling and capacity: node plugins and runtime"]
...
For **Inference autoscaling and capacity**, the mechanisms participating in that loop are Queue depth/age, Predicted token work, TTFT/inter-token, KV cache pressure, HPA/KEDA, Node autoscaler, Cold path, Warm buffer, Scale down, Capacity fallback. Some run synchronously on the caller's request, while others converge later. This timing distinction explains many production surprises: the desired object can exist before capacity is ready, a data path can continue while its control plane is impaired, and a timeout can leave the final side effect unknown.
...
**What it is.** User latency separates queue/prefill from decode behavior.
...
**What it is.** Memory/concurrency saturation can predict rejection before GPU utilization.
...
**What it is.** Scales replicas from custom/external metrics with stabilization and activation.
...
**What it is.** Provisions GPU nodes only after replicas become Pending.
...
**What it is.** Cloud capacity, boot, driver/plugin, image, model download/load/warm dominate delay.
...
**What it is.** Preprovisioned nodes/replicas/cache trades cost for burst SLO.
...
**What it is.** Drain streams, protect cache/work, PDB/consolidation and minimum capacity.
...
**What it is.** Diversified approved hardware/Region/provider requires compatibility/quality/residency tests.
...
A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **Inference autoscaling and capacity**, not merely a list to copy.
...
Build a small, safe proof of **Inference autoscaling and capacity** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
