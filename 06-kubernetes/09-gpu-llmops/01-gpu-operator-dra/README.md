...
**GPU Operator, device plugins and DRA** is easiest to understand as one part of a larger path. The subject is governed through an API-driven feedback system. A client writes desired state, controllers compare it with observed state, the scheduler chooses placement, and node agents and plugins produce the running data plane.

The chapter focuses on Driver, Container toolkit, Device plugin, GPU Feature Discovery. These are connected mechanisms, not vocabulary to memorize. Expose, allocate, monitor and lifecycle GPU devices through a qualified driver/runtime/Kubernetes integration The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **GPU Operator, device plugins and DRA** is the next layer of that evolution. Its modern purpose is to expose, allocate, monitor and lifecycle GPU devices through a qualified driver/runtime/Kubernetes integration. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["GPU Operator, device plugins and DRA: node plugins and runtime"]
...
**What it is.** Advertises integer extended resources and allocates device nodes.
...
**What it is.** Labels node hardware/capabilities for scheduling policy.
...
**What it is.** Reconciles driver/toolkit/plugin/GFD/DCGM/MIG components under one version matrix.
...
**What it is.** Driver publishes device inventory/attributes/topology.
...
**What it is.** Admin policy selects device characteristics using CEL.
...
**What it is.** Workload requests and records allocated/prepared device.
...
**What it is.** Unhealthy device reduces future allocatable or reports claim/Pod status under mechanism.
...
**What it is.** Drain one pool, update driver/operator/runtime, qualify CUDA/NCCL/model then expand.
...
- `kubectl get nodes -o custom-columns='NAME:.metadata.name,GPU:.status.allocatable.nvidia\.com/gpu'` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl get deviceclass,resourceslice,resourceclaim -A` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl exec -n ai POD -- nvidia-smi` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **GPU Operator, device plugins and DRA**, not merely a list to copy.
...
Build a small, safe proof of **GPU Operator, device plugins and DRA** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
