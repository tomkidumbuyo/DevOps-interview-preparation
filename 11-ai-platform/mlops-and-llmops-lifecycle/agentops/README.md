...
Operate plans, state, tool calls and approvals as durable, authorized workflows rather than trusting generated text.

```mermaid
flowchart LR
  I[identity and desired state] --> C[AgentOps and tool-using system operations control plane]
  C --> D[AgentOps and tool-using system operations data plane]
...
For AgentOps and tool-using system operations, draw a real request/resource path and label where these mechanisms act: Run state machine, Tool contract, Authorization, Capability, Idempotency, Approval, Budgets, Sandbox, Trace, Evaluation. State which parts are control plane versus data plane, regional versus zonal/global, synchronous versus asynchronous, and customer versus provider responsibility.
...
1. **Easy:** inventory one healthy AgentOps and tool-using system operations resource and draw identity/control/data/dependency paths.
...
Explain AgentOps and tool-using system operations in five passes: purpose/selection, mechanism/lifecycle, security/failure, operation/commands, and architecture/economics. Then complete the separate [answered question bank](questions-and-answers.md) without looking at these notes.
