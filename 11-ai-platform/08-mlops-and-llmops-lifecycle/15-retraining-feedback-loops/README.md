...
<!-- chapter-guide:start -->
> **Step 309 of 373 — 11.08.15**
>
> **Builds on:** [ML monitoring, drift and data quality](../14-monitoring-drift/README.md)
>
> **Now:** Learn **Retraining and feedback loops** from its mental model through production ownership.
>
> **Then:** Rehearse the linked questions and continue to [PromptOps: prompt, template and policy lifecycle](../16-promptops/README.md).
<!-- chapter-guide:end -->
...
**Retraining and feedback loops** is easiest to understand as one part of a larger path. The subject links data, model and runtime state. A versioned input becomes an artifact, a serving system turns requests into token or prediction work, and evaluation decides whether quality, safety, latency and cost are acceptable.

The chapter focuses on Trigger, Feedback collection, Label quality, Sampling bias. These are connected mechanisms, not vocabulary to memorize. Turn labeled outcomes and approved data changes into bounded retraining without poisoning, bias amplification or uncontrolled promotion The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **Retraining and feedback loops** is the next layer of that evolution. Its modern purpose is to turn labeled outcomes and approved data changes into bounded retraining without poisoning, bias amplification or uncontrolled promotion. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["Retraining and feedback loops: training or inference runtime"]
...
For **Retraining and feedback loops**, the mechanisms participating in that loop are Trigger, Feedback collection, Label quality, Sampling bias, Poisoning, Replay set, Pipeline version, Budget/admission, Validation, Human promotion. Some run synchronously on the caller's request, while others converge later. This timing distinction explains many production surprises: the desired object can exist before capacity is ready, a data path can continue while its control plane is impaired, and a timeout can leave the final side effect unknown.

### Core concepts explained in detail

#### Trigger

**What it is.** Schedule, data volume, drift, quality incident or business change proposes retraining under debounce/budget controls.
...
**What it is.** Stable prediction/outcome IDs, consent, provenance and label definition determine trustworthy supervision.
...
**What it is.** Reviewer agreement, adjudication, sampling and leakage tests are part of training evidence.
...
**What it is.** Observed feedback reflects product exposure and user response rather than the target population automatically.
...
**What it is.** Anomalous contributors/content and influence controls protect training/evaluation sets.
...
**What it is.** Retain representative historical/edge cases so adaptation does not catastrophically forget.
...
**What it is.** Retraining uses immutable data/code/config/environment and produces a new candidate, never silent in-place mutation.
...
**What it is.** GPU, provider and pipeline quotas prevent feedback automation from starving serving.
...
**What it is.** Candidate must beat champion within owned quality/safety/performance/cost gates.
...
**What it is.** High-risk changes retain approval, exception and rollback rather than automatic drift-to-production.
...
```bash
python scripts/build_feedback_dataset.py --dry-run
dvc repro retrain
mlflow runs list --experiment-id EXPERIMENT_ID
python -m evals.run --manifest candidate.yaml --dataset regression.jsonl
```

What the example demonstrates:

- `python scripts/build_feedback_dataset.py --dry-run` captures a read-oriented state snapshot that must be interpreted against a healthy baseline, the exact target and the next adjacent dependency.
- `dvc repro retrain` captures a read-oriented state snapshot that must be interpreted against a healthy baseline, the exact target and the next adjacent dependency.
...
- `python -m evals.run --manifest candidate.yaml --dataset regression.jsonl` captures a read-oriented state snapshot that must be interpreted against a healthy baseline, the exact target and the next adjacent dependency.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **Retraining and feedback loops**, not merely a list to copy.
...
|---:|---|---|
| 1 | **Trigger** | schedule, data volume, drift, quality incident or business change proposes retraining under debounce/budget controls |
| 2 | **Feedback collection** | stable prediction/outcome IDs, consent, provenance and label definition determine trustworthy supervision |
| 3 | **Label quality** | reviewer agreement, adjudication, sampling and leakage tests are part of training evidence |
| 4 | **Sampling bias** | observed feedback reflects product exposure and user response rather than the target population automatically |
| 5 | **Poisoning** | anomalous contributors/content and influence controls protect training/evaluation sets |
| 6 | **Replay set** | retain representative historical/edge cases so adaptation does not catastrophically forget |
| 7 | **Pipeline version** | retraining uses immutable data/code/config/environment and produces a new candidate, never silent in-place mutation |
| 8 | **Budget/admission** | GPU, provider and pipeline quotas prevent feedback automation from starving serving |
| 9 | **Validation** | candidate must beat champion within owned quality/safety/performance/cost gates |
| 10 | **Human promotion** | high-risk changes retain approval, exception and rollback rather than automatic drift-to-production |
...
Build a small, safe proof of **Retraining and feedback loops** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
...
**Reading path:** [← Back: ML monitoring, drift and data quality](../14-monitoring-drift/README.md) · [Questions](questions-and-answers.md) · [Next: PromptOps: prompt, template and policy lifecycle →](../16-promptops/README.md)
