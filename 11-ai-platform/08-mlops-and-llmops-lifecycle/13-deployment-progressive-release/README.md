# ML/LLM deployment and progressive release

<!-- chapter-guide:start -->
> **Step 307 of 373 — 11.08.13**
>
> **Builds on:** [Model validation and release gates](../12-model-validation-release-gates/README.md)
>
> **Now:** Learn **ML/LLM deployment and progressive release** from its mental model through production ownership.
>
> **Then:** Rehearse the linked questions and continue to [ML monitoring, drift and data quality](../14-monitoring-drift/README.md).
<!-- chapter-guide:end -->
...
**ML/LLM deployment and progressive release** is easiest to understand as one part of a larger path. The subject links data, model and runtime state. A versioned input becomes an artifact, a serving system turns requests into token or prediction work, and evaluation decides whether quality, safety, latency and cost are acceptable.

The chapter focuses on Release manifest, Shadow, Canary, Champion/challenger. These are connected mechanisms, not vocabulary to memorize. Promote immutable releases through shadow, canary, champion/challenger and rollback with infrastructure and quality gates The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **ML/LLM deployment and progressive release** is the next layer of that evolution. Its modern purpose is to promote immutable releases through shadow, canary, champion/challenger and rollback with infrastructure and quality gates. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["ML/LLM deployment and progressive release: training or inference runtime"]
...
For **ML/LLM deployment and progressive release**, the mechanisms participating in that loop are Release manifest, Shadow, Canary, Champion/challenger, Traffic unit, Warmup, Compatibility, Rollback, Evidence window, Audit. Some run synchronously on the caller's request, while others converge later. This timing distinction explains many production surprises: the desired object can exist before capacity is ready, a data path can continue while its control plane is impaired, and a timeout can leave the final side effect unknown.

### Core concepts explained in detail

#### Release manifest

**What it is.** Model, tokenizer, prompt, adapter, runtime, hardware, index, policy and evaluator versions form one deployable contract.
...
**What it is.** Copies production inputs without user-visible output and requires privacy, side-effect and cost controls.
...
**What it is.** Routes bounded real traffic with automatic infrastructure and task-quality abort thresholds.
...
**What it is.** Retains incumbent and candidate comparison under explicit promotion authority.
...
**What it is.** Request/user/session/tenant assignment and stickiness determine unbiased consistent comparison.
...
**What it is.** Model download/load/compile/cache and representative request warmup occur before traffic readiness.
...
**What it is.** Schema, client, cache, feature/index and persisted state must work during mixed-version rollout.
...
**What it is.** Prior bytes/config/index/policy/capacity remain available and correctness is reverified after reversal.
...
**What it is.** Rare quality/safety failures may require longer/sample-aware gates than latency/error metrics.
...
**What it is.** Release actor, evidence, traffic changes, exceptions, incidents and final deployed digest remain linked.
...
```bash
kubectl rollout status deployment/MODEL -n inference
kubectl rollout history deployment/MODEL -n inference
kubectl get inferenceservice MODEL -n inference -o yaml
curl -s http://MODEL/metrics | rg 'request|token|latency|error'
```

What the example demonstrates:

- `kubectl rollout status deployment/MODEL -n inference` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl rollout history deployment/MODEL -n inference` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `kubectl get inferenceservice MODEL -n inference -o yaml` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `curl -s http://MODEL/metrics | rg 'request|token|latency|error'` tests one part of the end-to-end request path and preserves protocol-level evidence such as resolution, connection, handshake, status or packet direction.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **ML/LLM deployment and progressive release**, not merely a list to copy.
...
|---:|---|---|
| 1 | **Release manifest** | model, tokenizer, prompt, adapter, runtime, hardware, index, policy and evaluator versions form one deployable contract |
| 2 | **Shadow** | copies production inputs without user-visible output and requires privacy, side-effect and cost controls |
| 3 | **Canary** | routes bounded real traffic with automatic infrastructure and task-quality abort thresholds |
| 4 | **Champion/challenger** | retains incumbent and candidate comparison under explicit promotion authority |
| 5 | **Traffic unit** | request/user/session/tenant assignment and stickiness determine unbiased consistent comparison |
| 6 | **Warmup** | model download/load/compile/cache and representative request warmup occur before traffic readiness |
| 7 | **Compatibility** | schema, client, cache, feature/index and persisted state must work during mixed-version rollout |
| 8 | **Rollback** | prior bytes/config/index/policy/capacity remain available and correctness is reverified after reversal |
| 9 | **Evidence window** | rare quality/safety failures may require longer/sample-aware gates than latency/error metrics |
| 10 | **Audit** | release actor, evidence, traffic changes, exceptions, incidents and final deployed digest remain linked |
...
Build a small, safe proof of **ML/LLM deployment and progressive release** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
...
**Reading path:** [← Back: Model validation and release gates](../12-model-validation-release-gates/README.md) · [Questions](questions-and-answers.md) · [Next: ML monitoring, drift and data quality →](../14-monitoring-drift/README.md)
