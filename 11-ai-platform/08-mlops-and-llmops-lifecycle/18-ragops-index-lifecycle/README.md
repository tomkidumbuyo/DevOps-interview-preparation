# RAGOps and index lifecycle

<!-- chapter-guide:start -->
> **Step 312 of 373 — 11.08.18**
>
> **Builds on:** [LLM fine-tuning, PEFT and adapter operations](../17-llm-finetuning-peft/README.md)
>
> **Now:** Learn **RAGOps and index lifecycle** from its mental model through production ownership.
>
> **Then:** Rehearse the linked questions and continue to [EvalOps: continuous AI evaluation infrastructure](../19-evalops/README.md).
<!-- chapter-guide:end -->

> Interview bank: [questions-and-answers.md](questions-and-answers.md) · Official documentation: <https://docs.llamaindex.ai/>

## Explanation

### What it is and why it exists

**RAGOps and index lifecycle** is easiest to understand as one part of a larger path. The subject links data, model and runtime state. A versioned input becomes an artifact, a serving system turns requests into token or prediction work, and evaluation decides whether quality, safety, latency and cost are acceptable.

The chapter focuses on Source inventory, Ingestion idempotency, Parser/chunker version, Embedding release. These are connected mechanisms, not vocabulary to memorize. Reconcile source permissions and changes through parse, chunk, embed, index, retrieval, evaluation, deletion and rollback The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **RAGOps and index lifecycle** is the next layer of that evolution. Its modern purpose is to reconcile source permissions and changes through parse, chunk, embed, index, retrieval, evaluation, deletion and rollback. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["RAGOps and index lifecycle: training or inference runtime"]
...
For **RAGOps and index lifecycle**, the mechanisms participating in that loop are Source inventory, Ingestion idempotency, Parser/chunker version, Embedding release, Index build, Authorization, Freshness SLI, Deletion, Evaluation, Rollback. Some run synchronously on the caller's request, while others converge later. This timing distinction explains many production surprises: the desired object can exist before capacity is ready, a data path can continue while its control plane is impaired, and a timeout can leave the final side effect unknown.

### Core concepts explained in detail

#### Source inventory

**What it is.** Owner, connector, classification, ACL, residency, update/delete semantics and watermark are explicit.
...
**What it is.** Stable source/version IDs and checkpoints prevent duplicate or skipped documents.
...
**What it is.** Content extraction, tables/images and chunk boundaries change retrieval and require lineage.
...
**What it is.** Model/tokenizer/dimension/normalization and batching are immutable index dependencies.
...
**What it is.** Staging index is populated and validated before atomic alias/pointer promotion.
...
**What it is.** Tenant and source ACL filters apply before context is returned; application post-filtering is too late.
...
**What it is.** Source update→ingest→embed→index→query visibility latency is measured by stage.
...
**What it is.** Tombstone and hard-delete reconcile chunks, vector/sparse indexes, caches, evaluations and backups under policy.
...
**What it is.** Retrieval recall/precision/MRR/nDCG and answer faithfulness/citation/quality are separated by slices.
...
**What it is.** Retain compatible prior index and query code; alias reversal must not reintroduce deleted/unauthorized content.
...
```bash
python ingest.py --source fixtures/docs --dry-run
python build_index.py --manifest corpus-v12.json --target staging-v12
python evaluate_rag.py --dataset eval/rag-golden.jsonl
curl -s VECTOR_DB/health
```

What the example demonstrates:

- `python ingest.py --source fixtures/docs --dry-run` captures a read-oriented state snapshot that must be interpreted against a healthy baseline, the exact target and the next adjacent dependency.
- `python build_index.py --manifest corpus-v12.json --target staging-v12` captures a read-oriented state snapshot that must be interpreted against a healthy baseline, the exact target and the next adjacent dependency.
- `python evaluate_rag.py --dataset eval/rag-golden.jsonl` captures a read-oriented state snapshot that must be interpreted against a healthy baseline, the exact target and the next adjacent dependency.
- `curl -s VECTOR_DB/health` tests one part of the end-to-end request path and preserves protocol-level evidence such as resolution, connection, handshake, status or packet direction.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **RAGOps and index lifecycle**, not merely a list to copy.
...
|---:|---|---|
| 1 | **Source inventory** | owner, connector, classification, ACL, residency, update/delete semantics and watermark are explicit |
| 2 | **Ingestion idempotency** | stable source/version IDs and checkpoints prevent duplicate or skipped documents |
| 3 | **Parser/chunker version** | content extraction, tables/images and chunk boundaries change retrieval and require lineage |
| 4 | **Embedding release** | model/tokenizer/dimension/normalization and batching are immutable index dependencies |
| 5 | **Index build** | staging index is populated and validated before atomic alias/pointer promotion |
| 6 | **Authorization** | tenant and source ACL filters apply before context is returned; application post-filtering is too late |
| 7 | **Freshness SLI** | source update→ingest→embed→index→query visibility latency is measured by stage |
| 8 | **Deletion** | tombstone and hard-delete reconcile chunks, vector/sparse indexes, caches, evaluations and backups under policy |
| 9 | **Evaluation** | retrieval recall/precision/MRR/nDCG and answer faithfulness/citation/quality are separated by slices |
| 10 | **Rollback** | retain compatible prior index and query code; alias reversal must not reintroduce deleted/unauthorized content |
...
Build a small, safe proof of **RAGOps and index lifecycle** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
...
**Reading path:** [← Back: LLM fine-tuning, PEFT and adapter operations](../17-llm-finetuning-peft/README.md) · [Questions](questions-and-answers.md) · [Next: EvalOps: continuous AI evaluation infrastructure →](../19-evalops/README.md)
