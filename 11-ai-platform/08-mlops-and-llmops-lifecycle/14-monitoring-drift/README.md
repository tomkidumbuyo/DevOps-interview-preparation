# ML monitoring, drift and data quality

<!-- chapter-guide:start -->
> **Step 308 of 373 — 11.08.14**
>
> **Builds on:** [ML/LLM deployment and progressive release](../13-deployment-progressive-release/README.md)
>
> **Now:** Learn **ML monitoring, drift and data quality** from its mental model through production ownership.
>
> **Then:** Rehearse the linked questions and continue to [Retraining and feedback loops](../15-retraining-feedback-loops/README.md).
<!-- chapter-guide:end -->

> Interview bank: [questions-and-answers.md](questions-and-answers.md) · Official documentation: <https://www.tensorflow.org/tfx/guide/tfdv>

## Explanation

### What it is and why it exists

**ML monitoring, drift and data quality** is easiest to understand as one part of a larger path. The subject links data, model and runtime state. A versioned input becomes an artifact, a serving system turns requests into token or prediction work, and evaluation decides whether quality, safety, latency and cost are acceptable.

The chapter focuses on Input quality, Covariate drift, Prediction drift, Concept drift. These are connected mechanisms, not vocabulary to memorize. Detect actionable changes in inputs, predictions, labels, quality and system behavior without confusing drift with root cause The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **ML monitoring, drift and data quality** is the next layer of that evolution. Its modern purpose is to detect actionable changes in inputs, predictions, labels, quality and system behavior without confusing drift with root cause. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["ML monitoring, drift and data quality: training or inference runtime"]
...
For **ML monitoring, drift and data quality**, the mechanisms participating in that loop are Input quality, Covariate drift, Prediction drift, Concept drift, Label delay, Slice monitoring, Threshold, Data lineage, Feedback loop, Response. Some run synchronously on the caller's request, while others converge later. This timing distinction explains many production surprises: the desired object can exist before capacity is ready, a data path can continue while its control plane is impaired, and a timeout can leave the final side effect unknown.

### Core concepts explained in detail

#### Input quality

**What it is.** Schema, missingness, ranges, categories and parse failures catch broken pipelines before abstract drift scores.
...
**What it is.** Production input distribution differs from reference but does not alone prove quality degradation.
...
**What it is.** Output distribution changes and can arise from traffic, model, policy or upstream shifts.
...
**What it is.** Relationship between input and desired output changes and requires trustworthy delayed labels.
...
**What it is.** Monitoring architecture reconciles predictions with outcomes later using stable privacy-safe IDs.
...
**What it is.** Aggregate stability can hide one tenant/language/segment regression.
...
**What it is.** Expected seasonality/sample size/uncertainty and business impact determine alerting rather than arbitrary distance.
...
**What it is.** Identify which source/transform/model deployments explain an observed change.
...
**What it is.** Model decisions can alter future data and create selection or self-reinforcement bias.
...
**What it is.** Investigate/evaluate/shadow/retrain/rollback under owned policy; never auto-retrain on one drift metric.
...
```bash
python scripts/validate_batch.py --reference reference.parquet --current current.parquet
curl -s http://MODEL/metrics
kubectl logs -n inference deploy/MODEL --since=30m
python -m pytest tests/drift -q
```

What the example demonstrates:

- `python scripts/validate_batch.py --reference reference.parquet --current current.parquet` captures a read-oriented state snapshot that must be interpreted against a healthy baseline, the exact target and the next adjacent dependency.
...
- `kubectl logs -n inference deploy/MODEL --since=30m` reads Kubernetes API desired/status state or recent reconciliation evidence without treating a single `Ready` value as proof of the user journey.
- `python -m pytest tests/drift -q` captures a read-oriented state snapshot that must be interpreted against a healthy baseline, the exact target and the next adjacent dependency.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **ML monitoring, drift and data quality**, not merely a list to copy.
...
|---:|---|---|
| 1 | **Input quality** | schema, missingness, ranges, categories and parse failures catch broken pipelines before abstract drift scores |
| 2 | **Covariate drift** | production input distribution differs from reference but does not alone prove quality degradation |
| 3 | **Prediction drift** | output distribution changes and can arise from traffic, model, policy or upstream shifts |
| 4 | **Concept drift** | relationship between input and desired output changes and requires trustworthy delayed labels |
| 5 | **Label delay** | monitoring architecture reconciles predictions with outcomes later using stable privacy-safe IDs |
| 6 | **Slice monitoring** | aggregate stability can hide one tenant/language/segment regression |
| 7 | **Threshold** | expected seasonality/sample size/uncertainty and business impact determine alerting rather than arbitrary distance |
| 8 | **Data lineage** | identify which source/transform/model deployments explain an observed change |
| 9 | **Feedback loop** | model decisions can alter future data and create selection or self-reinforcement bias |
| 10 | **Response** | investigate/evaluate/shadow/retrain/rollback under owned policy; never auto-retrain on one drift metric |
...
Build a small, safe proof of **ML monitoring, drift and data quality** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
...
**Reading path:** [← Back: ML/LLM deployment and progressive release](../13-deployment-progressive-release/README.md) · [Questions](questions-and-answers.md) · [Next: Retraining and feedback loops →](../15-retraining-feedback-loops/README.md)
