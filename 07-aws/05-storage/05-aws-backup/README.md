...
<!-- chapter-guide:start -->
> **Step 131 of 373 — 07.05.05**
>
> **Builds on:** [Amazon FSx](../04-fsx/README.md)
>
> **Now:** Learn **AWS Backup** from its mental model through production ownership.
>
> **Then:** Rehearse the linked questions and continue to [Containers](../../06-containers/README.md).
<!-- chapter-guide:end -->
...
**AWS Backup** is easiest to understand as one part of a larger path. The subject has an API control plane and a workload data plane. An authenticated request is authorized, validated and persisted; managed controllers then create or reconfigure regional or global resources that serve traffic or process data.

The chapter focuses on Backup plan, Resource selection, Backup vault, Vault Lock. These are connected mechanisms, not vocabulary to memorize. Apply immutable cross-account/Region backup policy and prove application recovery through automated restore tests The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **AWS Backup** is the next layer of that evolution. Its modern purpose is to apply immutable cross-account/Region backup policy and prove application recovery through automated restore tests. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["AWS Backup: regional or global data plane"]
...
For **AWS Backup**, the mechanisms participating in that loop are Backup plan, Resource selection, Backup vault, Vault Lock, Cross-account copy, Cross-Region copy, Recovery point, Restore testing, Backup Audit Manager, RPO/RTO evidence. Some run synchronously on the caller's request, while others converge later. This timing distinction explains many production surprises: the desired object can exist before capacity is ready, a data path can continue while its control plane is impaired, and a timeout can leave the final side effect unknown.

### Core concepts explained in detail

#### Backup plan

**What it is.** Schedule, window, lifecycle and copy rules define intended RPO/retention.
...
**What it is.** Tags/ARNs/roles determine coverage and need inventory validation.
...
**What it is.** Access/KMS/lock policies isolate recovery points from workloads.
...
**What it is.** Governance/compliance modes constrain deletion and require careful retention/legal operation.
...
**What it is.** Reduces workload-account compromise blast radius under organization/KMS policy.
...
**What it is.** Addresses regional disaster while adding transfer/residency considerations.
...
**What it is.** Provider-specific restore metadata and consistency determine usefulness.
...
**What it is.** Scheduled restore plus application validation measures actual recovery.
...
**What it is.** Produces control evidence but needs accurate framework/scope.
...
**What it is.** Job success, lag, copy and timed restore results must meet business targets.
...
- `aws backup list-backup-plans` queries the AWS service control plane; IDs, Region, status/reason fields and request attribution should be correlated with CloudTrail and service metrics.
- `aws backup list-backup-jobs` queries the AWS service control plane; IDs, Region, status/reason fields and request attribution should be correlated with CloudTrail and service metrics.
- `aws backup list-recovery-points-by-backup-vault --backup-vault-name VAULT` queries the AWS service control plane; IDs, Region, status/reason fields and request attribution should be correlated with CloudTrail and service metrics.
- `aws backup list-restore-testing-plans` queries the AWS service control plane; IDs, Region, status/reason fields and request attribution should be correlated with CloudTrail and service metrics.
- `aws backup list-restore-jobs` queries the AWS service control plane; IDs, Region, status/reason fields and request attribution should be correlated with CloudTrail and service metrics.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **AWS Backup**, not merely a list to copy.
...
Build a small, safe proof of **AWS Backup** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
...
**Reading path:** [← Back: Amazon FSx](../04-fsx/README.md) · [Questions](questions-and-answers.md) · [Next: Containers →](../../06-containers/README.md)
