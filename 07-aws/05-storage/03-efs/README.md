...
**Amazon EFS** is easiest to understand as one part of a larger path. The subject has an API control plane and a workload data plane. An authenticated request is authorized, validated and persisted; managed controllers then create or reconfigure regional or global resources that serve traffic or process data.

The chapter focuses on Mount target, NFS semantics, Access point, IAM/TLS mount. These are connected mechanisms, not vocabulary to memorize. Provide regional NFS with per-AZ mount targets, identity/permission controls and throughput-aware operations The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **Amazon EFS** is the next layer of that evolution. Its modern purpose is to provide regional NFS with per-AZ mount targets, identity/permission controls and throughput-aware operations. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["Amazon EFS: regional or global data plane"]
...
**What it is.** Shared file access, locking, metadata and close-to-open behavior differ from object/block storage.
...
**What it is.** Enforces root directory and POSIX identity for an application path.
...
**What it is.** Helper can authenticate/encrypt supported client mount sessions.
...
**What it is.** General/max-I/O choices trade latency and scale under service constraints.
...
**What it is.** Bursting/provisioned/elastic choices affect limits and billing.
...
**What it is.** Moves files based on access and incurs retrieval/transition considerations.
...
**What it is.** Clients should normally use a mount target in their AZ for availability/cost/latency.
...
**What it is.** UID/GID/mode/ACL plus access-point behavior determine file access.
...
**What it is.** Shared filesystem still needs application-consistent policy and restore testing.
...
- `aws efs describe-file-systems` queries the AWS service control plane; IDs, Region, status/reason fields and request attribution should be correlated with CloudTrail and service metrics.
- `aws efs describe-mount-targets --file-system-id FS_ID` queries the AWS service control plane; IDs, Region, status/reason fields and request attribution should be correlated with CloudTrail and service metrics.
- `aws efs describe-access-points --file-system-id FS_ID` queries the AWS service control plane; IDs, Region, status/reason fields and request attribution should be correlated with CloudTrail and service metrics.
- `mount | rg nfs && nfsstat -m` captures a read-oriented state snapshot that must be interpreted against a healthy baseline, the exact target and the next adjacent dependency.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **Amazon EFS**, not merely a list to copy.
...
Build a small, safe proof of **Amazon EFS** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
