...
<!-- chapter-guide:start -->
> **Step 125 of 373 — 07.04.04**
>
> **Builds on:** [Gateway Load Balancer](../03-gwlb/README.md)
>
> **Now:** Learn **AWS Global Accelerator** from its mental model through production ownership.
>
> **Then:** Rehearse the linked questions and continue to [Storage](../../05-storage/README.md).
<!-- chapter-guide:end -->

> Interview bank: [questions-and-answers.md](questions-and-answers.md) · Official documentation: <https://docs.aws.amazon.com/global-accelerator/latest/dg/what-is-global-accelerator.html>

## Explanation

### What it is and why it exists

**AWS Global Accelerator** is easiest to understand as one part of a larger path. The subject has an API control plane and a workload data plane. An authenticated request is authorized, validated and persisted; managed controllers then create or reconfigure regional or global resources that serve traffic or process data.

The chapter focuses on Anycast IP, Accelerator listener, Endpoint group, Endpoint weight. These are connected mechanisms, not vocabulary to memorize. Route users through AWS anycast and network paths to healthy regional endpoints with deterministic addresses The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **AWS Global Accelerator** is the next layer of that evolution. Its modern purpose is to route users through AWS anycast and network paths to healthy regional endpoints with deterministic addresses. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["AWS Global Accelerator: regional or global data plane"]
...
For **AWS Global Accelerator**, the mechanisms participating in that loop are Anycast IP, Accelerator listener, Endpoint group, Endpoint weight, Health failover, Client IP preservation, Traffic dial, ALB/NLB/EC2/EIP endpoints, DNS vs anycast, Residency/state. Some run synchronously on the caller's request, while others converge later. This timing distinction explains many production surprises: the desired object can exist before capacity is ready, a data path can continue while its control plane is impaired, and a timeout can leave the final side effect unknown.

### Core concepts explained in detail

#### Anycast IP

**What it is.** The same static addresses advertise globally and enter a nearby AWS edge.
...
**What it is.** Accepts TCP/UDP port ranges and distributes to endpoint groups.
...
**What it is.** Regional endpoints, traffic dial and health/check settings.
...
**What it is.** Shifts traffic among endpoints within a group under health/capacity policy.
...
**What it is.** Unhealthy endpoints/regions are removed under detection thresholds and state implications.
...
**What it is.** Supported endpoint patterns retain source information under constraints.
...
**What it is.** Controls percentage sent to a Region for canary/maintenance/capacity.
...
**What it is.** Supported endpoint types provide different L7/L4 and address properties.
...
**What it is.** Global Accelerator reacts at network routing without waiting for client DNS TTL.
...
**What it is.** Global routing must not send data to a disallowed Region or unprepared replica.
...
aws globalaccelerator list-endpoint-groups --listener-arn ARN --region us-west-2
aws globalaccelerator describe-endpoint-group --endpoint-group-arn ARN --region us-west-2
```

What the example demonstrates:

- `aws globalaccelerator list-accelerators --region us-west-2` queries the AWS service control plane; IDs, Region, status/reason fields and request attribution should be correlated with CloudTrail and service metrics.
- `aws globalaccelerator list-listeners --accelerator-arn ARN --region us-west-2` queries the AWS service control plane; IDs, Region, status/reason fields and request attribution should be correlated with CloudTrail and service metrics.
- `aws globalaccelerator list-endpoint-groups --listener-arn ARN --region us-west-2` queries the AWS service control plane; IDs, Region, status/reason fields and request attribution should be correlated with CloudTrail and service metrics.
- `aws globalaccelerator describe-endpoint-group --endpoint-group-arn ARN --region us-west-2` queries the AWS service control plane; IDs, Region, status/reason fields and request attribution should be correlated with CloudTrail and service metrics.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **AWS Global Accelerator**, not merely a list to copy.
...
|---:|---|---|
| 1 | **Anycast IP** | the same static addresses advertise globally and enter a nearby AWS edge |
| 2 | **Accelerator listener** | accepts TCP/UDP port ranges and distributes to endpoint groups |
| 3 | **Endpoint group** | regional endpoints, traffic dial and health/check settings |
| 4 | **Endpoint weight** | shifts traffic among endpoints within a group under health/capacity policy |
| 5 | **Health failover** | unhealthy endpoints/regions are removed under detection thresholds and state implications |
| 6 | **Client IP preservation** | supported endpoint patterns retain source information under constraints |
| 7 | **Traffic dial** | controls percentage sent to a Region for canary/maintenance/capacity |
| 8 | **ALB/NLB/EC2/EIP endpoints** | supported endpoint types provide different L7/L4 and address properties |
| 9 | **DNS vs anycast** | Global Accelerator reacts at network routing without waiting for client DNS TTL |
| 10 | **Residency/state** | global routing must not send data to a disallowed Region or unprepared replica |
...
Build a small, safe proof of **AWS Global Accelerator** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
...
**Reading path:** [← Back: Gateway Load Balancer](../03-gwlb/README.md) · [Questions](questions-and-answers.md) · [Next: Storage →](../../05-storage/README.md)
