...
<!-- chapter-guide:start -->
> **Step 138 of 373 — 07.07**
>
> **Builds on:** [AWS Load Balancer Controller](../06-containers/05-aws-load-balancer-controller/README.md)
>
> **Now:** Learn **Databases** from its mental model through production ownership.
>
> **Then:** Rehearse the linked questions and continue to [Amazon RDS](01-rds/README.md).
<!-- chapter-guide:end -->
...
flowchart LR
  ...
  B --> S1[Amazon RDS]
  B --> S2[Amazon Aurora]
  B --> S3[Amazon DynamoDB]
  B --> S4[Amazon ElastiCache]
  B --> S5[Amazon OpenSearch Service]
```

## Branch learning contract

Learn the easy mental model first, run the read-only commands in a sandbox, render/apply the examples only in disposable environments, then break and repair one dependency at a time. Be able to connect these topics across the branch: DB instance, Multi-AZ, Read replica, Cluster volume, Writer endpoint, Reader endpoint, Partition key, Sort key, GSI, Valkey/Redis replication group, Cluster mode, Memcached, Index/mapping, Primary shard, Replica shard.
...
**Databases** is easiest to understand as one part of a larger path. The subject has an API control plane and a workload data plane. An authenticated request is authorized, validated and persisted; managed controllers then create or reconfigure regional or global resources that serve traffic or process data.

The chapter focuses on DB instance, Multi-AZ, Cluster volume, Writer endpoint. These are connected mechanisms, not vocabulary to memorize. Integrate the databases branch as one production capability rather than isolated products The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **Databases** is the next layer of that evolution. Its modern purpose is to integrate the databases branch as one production capability rather than isolated products. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["Databases: regional or global data plane"]
...
**What it is.** Synchronous standby/failover for availability rather than read scaling.
...
**What it is.** Replicated distributed storage is shared by writer/readers.
...
**What it is.** Follows current writer and should be used for write-capable connections.
...
**What it is.** Primary and replicas provide failover/read paths under node/AZ design.
...
**What it is.** Shards keys across node groups and requires compatible clients/key strategy.
...
**What it is.** Schema and analyzers determine searchable fields and cannot always be changed in place.
...
**What it is.** Unit of index distribution whose count affects scale, recovery and overhead.
...
- `aws rds describe-db-instances --db-instance-identifier DB` queries the AWS service control plane; IDs, Region, status/reason fields and request attribution should be correlated with CloudTrail and service metrics.
- `aws rds describe-db-clusters --db-cluster-identifier CLUSTER` queries the AWS service control plane; IDs, Region, status/reason fields and request attribution should be correlated with CloudTrail and service metrics.
...
- `aws elasticache describe-replication-groups` queries the AWS service control plane; IDs, Region, status/reason fields and request attribution should be correlated with CloudTrail and service metrics.
- `aws opensearch describe-domain --domain-name DOMAIN` queries the AWS service control plane; IDs, Region, status/reason fields and request attribution should be correlated with CloudTrail and service metrics.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **Databases**, not merely a list to copy.
...
Build a small, safe proof of **Databases** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
...
**Reading path:** [← Back: AWS Load Balancer Controller](../06-containers/05-aws-load-balancer-controller/README.md) · [Questions](questions-and-answers.md) · [Next: Amazon RDS →](01-rds/README.md)
