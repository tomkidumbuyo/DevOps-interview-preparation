...
<!-- chapter-guide:start -->
> **Step 141 of 373 — 07.07.03**
>
> **Builds on:** [Amazon Aurora](../02-aurora/README.md)
>
> **Now:** Learn **Amazon DynamoDB** from its mental model through production ownership.
>
> **Then:** Rehearse the linked questions and continue to [Amazon ElastiCache](../04-elasticache/README.md).
<!-- chapter-guide:end -->

> Interview bank: [questions-and-answers.md](questions-and-answers.md) · Official documentation: <https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Introduction.html>

## Explanation

### What it is and why it exists

**Amazon DynamoDB** is easiest to understand as one part of a larger path. The subject has an API control plane and a workload data plane. An authenticated request is authorized, validated and persisted; managed controllers then create or reconfigure regional or global resources that serve traffic or process data.

The chapter focuses on Partition key, Sort key, GSI, LSI. These are connected mechanisms, not vocabulary to memorize. Design access-pattern-first keys, capacity, indexes, streams and global replication without hot partitions The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **Amazon DynamoDB** is the next layer of that evolution. Its modern purpose is to design access-pattern-first keys, capacity, indexes, streams and global replication without hot partitions. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["Amazon DynamoDB: regional or global data plane"]
...
For **Amazon DynamoDB**, the mechanisms participating in that loop are Partition key, Sort key, GSI, LSI, On-demand/provisioned, Conditional write, Transaction, Streams, Global Tables, TTL. Some run synchronously on the caller's request, while others converge later. This timing distinction explains many production surprises: the desired object can exist before capacity is ready, a data path can continue while its control plane is impaired, and a timeout can leave the final side effect unknown.

### Core concepts explained in detail

#### Partition key

**What it is.** Hashes items across physical partitions and must distribute request volume.
...
**What it is.** Orders items within a partition and enables range/prefix access patterns.
...
**What it is.** Asynchronous alternate partition/sort key with its own storage/capacity/failure behavior.
...
**What it is.** Alternate sort key sharing table partition key and creation-time/size constraints.
...
**What it is.** Capacity modes trade predictability/control and can still throttle hot keys.
...
**What it is.** Atomically enforces version/idempotency/state transition predicates.
...
**What it is.** Coordinates multiple items at higher cost/limits and is not a relational query model.
...
**What it is.** Ordered per-item change records feed consumers under retention/idempotency needs.
...
**What it is.** Multi-Region active replication with conflict resolution semantics.
...
**What it is.** Asynchronous best-effort expiration; applications must not assume immediate deletion.
...
aws dynamodb query --table-name TABLE --key-condition-expression EXPR --expression-attribute-values file://values.json
aws dynamodb describe-continuous-backups --table-name TABLE
aws dynamodb describe-kinesis-streaming-destination --table-name TABLE
```

What the example demonstrates:

- `aws dynamodb describe-table --table-name TABLE` queries the AWS service control plane; IDs, Region, status/reason fields and request attribution should be correlated with CloudTrail and service metrics.
- `aws dynamodb query --table-name TABLE --key-condition-expression EXPR --expression-attribute-values file://values.json` queries the AWS service control plane; IDs, Region, status/reason fields and request attribution should be correlated with CloudTrail and service metrics.
- `aws dynamodb describe-continuous-backups --table-name TABLE` queries the AWS service control plane; IDs, Region, status/reason fields and request attribution should be correlated with CloudTrail and service metrics.
- `aws dynamodb describe-kinesis-streaming-destination --table-name TABLE` queries the AWS service control plane; IDs, Region, status/reason fields and request attribution should be correlated with CloudTrail and service metrics.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **Amazon DynamoDB**, not merely a list to copy.
...
|---:|---|---|
| 1 | **Partition key** | hashes items across physical partitions and must distribute request volume |
| 2 | **Sort key** | orders items within a partition and enables range/prefix access patterns |
| 3 | **GSI** | asynchronous alternate partition/sort key with its own storage/capacity/failure behavior |
| 4 | **LSI** | alternate sort key sharing table partition key and creation-time/size constraints |
| 5 | **On-demand/provisioned** | capacity modes trade predictability/control and can still throttle hot keys |
| 6 | **Conditional write** | atomically enforces version/idempotency/state transition predicates |
| 7 | **Transaction** | coordinates multiple items at higher cost/limits and is not a relational query model |
| 8 | **Streams** | ordered per-item change records feed consumers under retention/idempotency needs |
| 9 | **Global Tables** | multi-Region active replication with conflict resolution semantics |
| 10 | **TTL** | asynchronous best-effort expiration; applications must not assume immediate deletion |
...
Build a small, safe proof of **Amazon DynamoDB** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
...
**Reading path:** [← Back: Amazon Aurora](../02-aurora/README.md) · [Questions](questions-and-answers.md) · [Next: Amazon ElastiCache →](../04-elasticache/README.md)
