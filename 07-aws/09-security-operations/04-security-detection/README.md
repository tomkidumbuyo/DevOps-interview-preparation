# GuardDuty, Security Hub, Inspector, Macie and Detective

<!-- chapter-guide:start -->
> **Step 155 of 373 — 07.09.04**
>
> **Builds on:** [ACM, AWS WAF and Shield](../03-acm-waf-shield/README.md)
>
> **Now:** Learn **GuardDuty, Security Hub, Inspector, Macie and Detective** from its mental model through production ownership.
>
> **Then:** Rehearse the linked questions and continue to [Amazon CloudWatch and X-Ray](../05-cloudwatch-xray/README.md).
<!-- chapter-guide:end -->

> Interview bank: [questions-and-answers.md](questions-and-answers.md) · Official documentation: <https://docs.aws.amazon.com/guardduty/latest/ug/what-is-guardduty.html>

## Explanation

### What it is and why it exists

**GuardDuty, Security Hub, Inspector, Macie and Detective** is easiest to understand as one part of a larger path. The subject has an API control plane and a workload data plane. An authenticated request is authorized, validated and persisted; managed controllers then create or reconfigure regional or global resources that serve traffic or process data.

The chapter focuses on GuardDuty, Security Hub, Inspector, Macie. These are connected mechanisms, not vocabulary to memorize. Turn cloud security findings into owned, prioritized, evidence-preserving response rather than alert accumulation The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **GuardDuty, Security Hub, Inspector, Macie and Detective** is the next layer of that evolution. Its modern purpose is to turn cloud security findings into owned, prioritized, evidence-preserving response rather than alert accumulation. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["GuardDuty, Security Hub, Inspector, Macie and Detective: regional or global data plane"]
...
For **GuardDuty, Security Hub, Inspector, Macie and Detective**, the mechanisms participating in that loop are GuardDuty, Security Hub, Inspector, Macie, Detective, Delegated administration, Finding severity, Suppression, Automation, Coverage evidence. Some run synchronously on the caller's request, while others converge later. This timing distinction explains many production surprises: the desired object can exist before capacity is ready, a data path can continue while its control plane is impaired, and a timeout can leave the final side effect unknown.

### Core concepts explained in detail

#### GuardDuty

**What it is.** Analyzes supported logs/signals to emit contextual threat findings.
...
**What it is.** Aggregates/normalizes findings and control standards across accounts/Regions.
...
**What it is.** Continuously assesses supported compute/container/function packages and exposure.
...
**What it is.** Discovers/classifies sensitive S3 data and bucket access risk.
...
**What it is.** Correlates activity/entities to support investigation.
...
**What it is.** Central security account manages organization coverage without using management account.
...
**What it is.** Vendor score must be combined with asset exposure, data and exploitability.
...
**What it is.** Tuned archival/suppression needs owner/reason/expiry to avoid hiding new risk.
...
**What it is.** EventBridge/Security Hub actions can quarantine or ticket with rollback/safety controls.
...
**What it is.** Inventory accounts/Regions/services and detect disabled sensors or log gaps.
...
aws inspector2 list-findings --filter-criteria file://criteria.json
aws macie2 get-macie-session
```

What the example demonstrates:

- `aws guardduty list-detectors` queries the AWS service control plane; IDs, Region, status/reason fields and request attribution should be correlated with CloudTrail and service metrics.
- `aws securityhub get-findings --filters file://filters.json` queries the AWS service control plane; IDs, Region, status/reason fields and request attribution should be correlated with CloudTrail and service metrics.
- `aws inspector2 list-findings --filter-criteria file://criteria.json` queries the AWS service control plane; IDs, Region, status/reason fields and request attribution should be correlated with CloudTrail and service metrics.
- `aws macie2 get-macie-session` queries the AWS service control plane; IDs, Region, status/reason fields and request attribution should be correlated with CloudTrail and service metrics.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **GuardDuty, Security Hub, Inspector, Macie and Detective**, not merely a list to copy.
...
|---:|---|---|
| 1 | **GuardDuty** | analyzes supported logs/signals to emit contextual threat findings |
| 2 | **Security Hub** | aggregates/normalizes findings and control standards across accounts/Regions |
| 3 | **Inspector** | continuously assesses supported compute/container/function packages and exposure |
| 4 | **Macie** | discovers/classifies sensitive S3 data and bucket access risk |
| 5 | **Detective** | correlates activity/entities to support investigation |
| 6 | **Delegated administration** | central security account manages organization coverage without using management account |
| 7 | **Finding severity** | vendor score must be combined with asset exposure, data and exploitability |
| 8 | **Suppression** | tuned archival/suppression needs owner/reason/expiry to avoid hiding new risk |
| 9 | **Automation** | EventBridge/Security Hub actions can quarantine or ticket with rollback/safety controls |
| 10 | **Coverage evidence** | inventory accounts/Regions/services and detect disabled sensors or log gaps |
...
Build a small, safe proof of **GuardDuty, Security Hub, Inspector, Macie and Detective** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
...
**Reading path:** [← Back: ACM, AWS WAF and Shield](../03-acm-waf-shield/README.md) · [Questions](questions-and-answers.md) · [Next: Amazon CloudWatch and X-Ray →](../05-cloudwatch-xray/README.md)
