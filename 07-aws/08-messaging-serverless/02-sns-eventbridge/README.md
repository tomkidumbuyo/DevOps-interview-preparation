...
**Amazon SNS and EventBridge** is easiest to understand as one part of a larger path. The subject has an API control plane and a workload data plane. An authenticated request is authorized, validated and persisted; managed controllers then create or reconfigure regional or global resources that serve traffic or process data.

The chapter focuses on SNS topic, Subscription filter, Delivery retry/DLQ, EventBridge bus. These are connected mechanisms, not vocabulary to memorize. Fan out notifications and route domain events with filters, retries, archives and controlled cross-account policy The explanations below first build the simple model, then add the exact system behavior and production consequences.
...
In this chapter, **Amazon SNS and EventBridge** is the next layer of that evolution. Its modern purpose is to fan out notifications and route domain events with filters, retries, archives and controlled cross-account policy. The exact product surface may change by version, but the underlying state, request path and failure boundaries remain the durable ideas to learn.
...
flowchart LR
  ...
  B --> C["Amazon SNS and EventBridge: regional or global data plane"]
...
**What it is.** Endpoint-specific retry and dead-letter behavior must be observed.
...
**What it is.** Account/custom/partner buses receive events under resource policy.
...
**What it is.** Matches structured event fields and routes to targets.
...
**What it is.** Reshapes event for a target but creates schema coupling.
...
**What it is.** Retained bus events can be replayed with duplicate/side-effect controls.
...
**What it is.** Documents/discovers schemas but compatibility governance remains owned.
...
**What it is.** Organization/resource policies and target roles define trust.
...
**What it is.** Invokes targets on time patterns with retry/DLQ and idempotency requirements.
...
- `aws sns list-subscriptions-by-topic --topic-arn ARN` queries the AWS service control plane; IDs, Region, status/reason fields and request attribution should be correlated with CloudTrail and service metrics.
- `aws events list-event-buses` queries the AWS service control plane; IDs, Region, status/reason fields and request attribution should be correlated with CloudTrail and service metrics.
- `aws events list-rules --event-bus-name BUS` queries the AWS service control plane; IDs, Region, status/reason fields and request attribution should be correlated with CloudTrail and service metrics.
- `aws events test-event-pattern --event-pattern file://pattern.json --event file://event.json` queries the AWS service control plane; IDs, Region, status/reason fields and request attribution should be correlated with CloudTrail and service metrics.

A healthy run returns the intended identity/context, exits successfully and shows the expected object or response without a new warning, retry loop or saturation signal. A failure is useful evidence: preserve the exact exit code, status/reason, timestamp and target, then inspect the immediately adjacent layer before changing anything. This makes the example part of the explanation of **Amazon SNS and EventBridge**, not merely a list to copy.
...
Build a small, safe proof of **Amazon SNS and EventBridge** and explain the result in your own words. The goal is not command completion; it is to connect input, internal mechanism, observable state and user outcome.
