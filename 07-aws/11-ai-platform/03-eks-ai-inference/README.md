...
Serve and train models on EKS with qualified GPU nodes, Karpenter, model caches, queues and inference SLOs.

```mermaid
flowchart LR
  I[identity and desired state] --> C[AI/ML workloads on Amazon EKS control plane]
  C --> D[AI/ML workloads on Amazon EKS data plane]
...
For AI/ML workloads on Amazon EKS, draw a real request/resource path and label where these mechanisms act: GPU node pool, Karpenter GPU provisioning, GPU Operator/device plugin, KServe/vLLM/Triton, Model cache, Queue-based scaling, EFA/NCCL, Spot/capacity, Inference telemetry, Canary lifecycle. State which parts are control plane versus data plane, regional versus zonal/global, synchronous versus asynchronous, and customer versus provider responsibility.
...
1. **Easy:** inventory one healthy AI/ML workloads on Amazon EKS resource and draw identity/control/data/dependency paths.
...
Explain AI/ML workloads on Amazon EKS in five passes: purpose/selection, mechanism/lifecycle, security/failure, operation/commands, and architecture/economics. Then complete the separate [answered question bank](questions-and-answers.md) without looking at these notes.
